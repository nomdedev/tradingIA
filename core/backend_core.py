...
class DataManager:
    """
    Manages data fetching, caching, and validation for trading operations.
    
    Attributes:
        api: Alpaca API client instance
    ...
    def __init__(
        ...
        logging.basicConfig(
            level=logging.INFO, filename=str(log_path), format="%(asctime)s - %(levelname)s - %(message)s"
        )

        # Initialize Alpaca API if credentials are available
        if self.api_key and self.secret_key:
            try:
                self.api = tradeapi.REST(self.api_key, self.secret_key, ALPACA_CONFIG["base_url"], api_version="v2")
    ...
    def load_alpaca_data(
        ...
        """
        Load historical data from Alpaca API with caching.
        
        Args:
            symbol: Trading symbol (e.g., "BTC/USD")
            start_date: Start date in YYYY-MM-DD format
            end_date: End date in YYYY-MM-DD format (default: today)
            timeframe: Data timeframe ("5Min", "15Min", "1H", "1D")
            
        Returns:
            DataFrame with OHLCV data and calculated indicators
            
        Raises:
            Exception: If API fails or no data returned
        """
        try:
            if end_date is None:
            ...
            bars = self.api.get_crypto_bars(
                symbol, alpaca_timeframe, start=start.strftime("%Y-%m-%d"), end=end.strftime("%Y-%m-%d")
            ).df
    ...
    def resample_multi_tf(self, df_5m):
        try:
            ...
            return {"5min": df_5m, "15min": df_15m, "1h": df_1h}
            ...
            if "timestamp" not in df_mapped.columns:
                if isinstance(df_mapped.index, pd.DatetimeIndex):
                    ...
                    df_mapped["timestamp"] = pd.date_range(
                        start="2020-01-01", periods=len(df_mapped), freq="5min", tz="UTC"
                    )
    ...
    def validate_ohlc(self, df, auto_correct=False):
        ...
        if not invalid_close_high.empty:
            if auto_correct:
                self.logger.warning(f"Auto-correcting {len(invalid_close_high)} Close > High violations")
                df.loc[df["Close"] > df["High"], "High"] = df.loc[df["Close"] > df["High"], "Close"]
            else:
                raise ValueError(f"Close must be <= High. Found {len(invalid_close_high)} invalid bars")

        invalid_close_low = df[df["Close"] < df["Low"]]
        if not invalid_close_low.empty:
            if auto_correct:
                self.logger.warning(f"Auto-correcting {len(invalid_close_low)} Close < Low violations")
                df.loc[df["Close"] < df["Low"], "Low"] = df.loc[df["Close"] < df["Low"], "Close"]
            else:
                raise ValueError(f"Close must be >= Low. Found {len(invalid_close_low)} invalid bars")

        # Check Open within [Low, High]
        # AUDITORÍA FIX: Respetar auto_correct correctamente
        invalid_open_high = df[df["Open"] > df["High"]]
        if not invalid_open_high.empty:
            if auto_correct:
                self.logger.warning(f"Auto-correcting {len(invalid_open_high)} Open > High violations")
                df.loc[df["Open"] > df["High"], "High"] = df.loc[df["Open"] > df["High"], "Open"]
            else:
                raise ValueError(f"Open must be <= High. Found {len(invalid_open_high)} invalid bars")

        invalid_open_low = df[df["Open"] < df["Low"]]
        if not invalid_open_low.empty:
            if auto_correct:
                self.logger.warning(f"Auto-correcting {len(invalid_open_low)} Open < Low violations")
                df.loc[df["Open"] < df["Low"], "Low"] = df.loc[df["Open"] < df["Low"], "Open"]
            else:
                raise ValueError(f"Open must be >= Low. Found {len(invalid_open_low)} invalid bars")
    ...
    def validate_volume(self, df):
        ...
        if not zero_vol.empty:
            self.logger.warning(f"Zero volume detected in {len(zero_vol)} bars ({len(zero_vol)/len(df)*100:.1f}%)")
    ...
    def validate_no_future_data(self, df, tolerance_minutes=5):
        ...
        if not future_data.empty:
            raise ValueError(f"Future data detected: {len(future_data)} bars beyond {now + tolerance}")
    ...
    def handle_zero_volume(self, df, method="interpolate"):
        ...
        if method == "interpolate":
            ...
            # AUDITORÍA FIX: Usar ffill() en lugar de fillna(method="ffill") deprecado
    ...
    def calculate_sma_chunked(self, df, period=200, chunk_size=50000):
        ...
        for i in range(0, len(df), chunk_size):
            ...
            if i > 0 and period > 1:
                ...
                for j in range(min(period - 1, len(chunk))):
                    window_data = pd.concat([prev_values.iloc[-(period - j - 1) :], chunk.iloc[: j + 1]["Close"]])
        ...
        self.logger.info(f"Calculated SMA_{period} on {len(df)} bars in {len(result_chunks)} chunks")
...
class StrategyEngine:
    ...
    def validate_params(self, strategy_name, params_dict):
        ...
        for param_name, param_config in config_params.items():
            ...
            if not min_val <= value <= max_val:
                range_msg = f"Parameter {param_name} out of range: {value} " f"not in [{min_val}, {max_val}]"
    ...
    def _validate_param_dependencies(self, strategy_name, params_dict):
        ...
        if strategy_name == "Pairs":
            if "lookback" in params_dict and "entry_threshold" in params_dict:
                ...
                if lookback < 20:
                    return False, (f"Pairs lookback ({lookback}) must be >= 20 " "for stable spread calculation")
        ...
        if strategy_name == "LSTM_ML":
            if "sequence_length" in params_dict and "prediction_horizon" in params_dict:
                ...
                if pred_horizon > seq_length:
                    return False, (
                        f"LSTM prediction_horizon ({pred_horizon}) must be <= " f"sequence_length ({seq_length})"
                    )
    ...
    def validate_parameters(self, strategy_name: str, params_dict: dict) -> tuple:
        """
        Alias for validate_params for API consistency.
        
        Args:
            strategy_name: Name of the strategy to validate
            params_dict: Parameters dictionary to validate
            
        Returns:
            Tuple of (is_valid: bool, message: str)
        """
        return self.validate_params(strategy_name, params_dict)

    @staticmethod
    def validate_price_data(df: 'pd.DataFrame') -> tuple:
        """
        ...
        for col in required_cols:
            ...
            if neg_count > 0:
                issues.append(f"Column '{col}' has {neg_count} non-positive values")
        ...
        if invalid_open > 0:
            issues.append(f"{invalid_open} rows where open outside high-low range")
            ...
            "IBS_BB": {
                ...
                "params": {
                    ...
                    "vol_mult": {
                        ...
                        "max": 2.0,
                        "step": 0.1,
                        "description": "Volume multiplier for IBS filter",
                    },
                },
            },
            "MACD_ADX": {
            ...
            "LSTM_ML": {
                ...
                "params": {
                    "sequence_length": {
                        ...
                        "max": 120,
                        "step": 10,
                        "description": "LSTM sequence length",
                    },
                    "prediction_horizon": {
                        "default": 5,
                        "min": 1,
                        "max": 10,
                        "step": 1,
                        "description": "Prediction horizon in bars",
                    },
                },
            },
        }
