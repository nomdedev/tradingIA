...
class GeneticOptimizer:
    ...
    def evaluate_population_parallel(
        self, population: List[Individual], backtest_function: Callable
    ) -> List[Individual]:
        ...
        for param_name, value in mutated_genes.items():
            if random.random() < self.config.mutation_rate:
                ...
                if param_type == "int":
                    ...
                    new_index = max(0, min(len(values) - 1, current_index + random.choice([-1, 1])))
    ...
    def optimize(self, backtest_function: Callable, progress_callback: Optional[Callable] = None) -> Dict[str, Any]:
        """
...
class OptimizationManager:
    ...
    def add_strategy_optimization(
        self, strategy_name: str, strategy_class: Any, config: Optional[OptimizationConfig] = None
    ):
    ...
    def optimize_strategy(
        self, strategy_name: str, backtest_function: Callable, progress_callback: Optional[Callable] = None
    ) -> Dict[str, Any]:
    ...
    def optimize_all_strategies(
        self, backtest_functions: Dict[str, Callable], progress_callback: Optional[Callable] = None
    ) -> Dict[str, Dict]:
