...
class MomentumStrategy(BaseStrategy):
    ...
    def generate_signals(self, data: pd.DataFrame) -> pd.Series:
        ...
        for i in range(start_idx, len(data)):
            ...
            if pd.isna(row["fast_ma"]) or pd.isna(row["slow_ma"]) or pd.isna(row["momentum"]):
                continue
            ...
            # Momentum conditions
            momentum_up = row["momentum"] > momentum_threshold
            ma_crossover_up = (
                row["fast_ma"] > row["slow_ma"]
                and prev_row["fast_ma"] <= prev_row["slow_ma"]
            )
            rsi_oversold = row["rsi"] < 70  # Not overbought
            macd_positive = row["macd_hist"] > 0

            # Exit conditions
            momentum_down = row["momentum"] < -momentum_threshold
            ma_crossover_down = (
                row["fast_ma"] < row["slow_ma"]
                and prev_row["fast_ma"] >= prev_row["slow_ma"]
            )
