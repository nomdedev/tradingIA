...
class BreakoutStrategy(BaseStrategy):
    ...
    def generate_signals(self, data: pd.DataFrame) -> pd.Series:
        ...
        for i in range(start_idx, len(data)):
            ...
            # AUDITORÍA FIX: Skip si hay NaN en indicadores críticos
            if pd.isna(row["rolling_high"]) or pd.isna(row["rolling_low"]):
                continue
            if pd.isna(row["is_consolidating"]) or pd.isna(row["volume_ratio"]):
                continue
            ...
            consolidating = row["is_consolidating"]
            volume_spike = row["volume_ratio"] > 1.5  # 50% above average volume
            momentum_confirm = row["momentum"] > 0.01  # Positive momentum
            rsi_confirm = row["rsi"] > 50  # Not oversold
