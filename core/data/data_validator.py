...
class DataValidator:
    ...
    def validate_ohlc_relationships(self, df: pd.DataFrame) -> ValidationResult:
        ...
        return ValidationResult("ohlc_relationships", ValidationSeverity.INFO, True, "OHLC relationships are valid")
    ...
    def detect_time_gaps(self, df: pd.DataFrame, expected_freq: str = "5min") -> ValidationResult:
        ...
        return ValidationResult("time_gaps", ValidationSeverity.INFO, True, "No significant time gaps detected")
    ...
    def check_duplicate_timestamps(self, df: pd.DataFrame) -> ValidationResult:
        ...
        """
        if "timestamp" not in df.columns:
            return ValidationResult("duplicate_timestamps", ValidationSeverity.ERROR, False, "Missing timestamp column")
        ...
        if dup_count > 0:
            ...
            return ValidationResult(
                ...
                {"duplicate_count": dup_count, "duplicate_timestamps": list(dup_timestamps)},
                "Remove or aggregate duplicate timestamps",
            )

        return ValidationResult("duplicate_timestamps", ValidationSeverity.INFO, True, "No duplicate timestamps found")
    ...
    def validate_timezone(self, df: pd.DataFrame, expected_tz: str = "UTC") -> ValidationResult:
        ...
        """
        if "timestamp" not in df.columns:
            return ValidationResult("timezone", ValidationSeverity.ERROR, False, "Missing timestamp column")
        ...
        return ValidationResult(
            "timezone", ValidationSeverity.INFO, True, f"All timestamps are in {expected_tz} timezone"
        )
    ...
    def detect_look_ahead_bias(self, df: pd.DataFrame) -> ValidationResult:
        ...
        """
        if "timestamp" not in df.columns:
            return ValidationResult("look_ahead_bias", ValidationSeverity.ERROR, False, "Missing timestamp column")
        ...
        return ValidationResult("look_ahead_bias", ValidationSeverity.INFO, True, "No look-ahead bias detected")
    ...
    def validate_volume(self, df: pd.DataFrame) -> ValidationResult:
        ...
        if "volume" not in df.columns:
            return ValidationResult(
                "volume", ValidationSeverity.WARNING, True, "No volume column found (acceptable for some assets)"
            )
        ...
        return ValidationResult("volume", ValidationSeverity.INFO, True, "Volume data appears valid")
    ...
    def check_large_dataset(self, df: pd.DataFrame, max_rows: int = 100000) -> ValidationResult:
        ...
        if row_count > max_rows:
            return ValidationResult(
                ...
                {"row_count": row_count, "max_recommended": max_rows, "excess_rows": row_count - max_rows},
    ...
    def run_all_validations(
        self, df: pd.DataFrame, expected_freq: str = "5min", expected_tz: str = "UTC"
        ...
        for check in checks:
            try:
                ...
                self.results.append(
                    ValidationResult(
                        "validation_error", ValidationSeverity.ERROR, False, f"Validation check failed: {str(e)}"
                    )
                )
    ...
    def get_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "critical": critical,
            "results": [
                {"check": r.check_name, "severity": r.severity.value, "passed": r.passed, "message": r.message}
...
def validate_dataframe(
    df: pd.DataFrame, strict_mode: bool = False, expected_freq: str = "5min", expected_tz: str = "UTC"
) -> Dict[str, Any]:
    """
...
def get_council_context_from_validation(validation_summary: Dict[str, Any]) -> Dict[str, Any]:
    """
    ÁREA 7: Convert validation summary to Council context for Data Oracle.
    
    This allows the Council's Data Oracle to veto trades based on data quality.
    
    Args:
        validation_summary: Output from DataValidator.get_summary()
        
    Returns:
        dict: Context for Council.decide() with data quality information
    """
    if validation_summary.get("status") == "no_validations_run":
        return {
            "data_quality": {
                "validated": False,
                "status": "unknown",
                "score": 0.5,  # Neutral score if not validated
                "issues": ["Data not validated"]
            }
        }
    ...
    critical = validation_summary.get("critical", 0)
    warnings = validation_summary.get("warnings", 0)
    
    if total_checks == 0:
        score = 0.5
    else:
        # Weight: critical=-1.0, error=-0.5, warning=-0.1, pass=+0.2
        score_sum = passed * 0.2 - warnings * 0.1 - errors * 0.5 - critical * 1.0
        max_score = total_checks * 0.2
        score = max(0.0, min(1.0, (score_sum / max_score) if max_score > 0 else 0.5))
    
    # Collect issues
    issues = []
    for result in validation_summary.get("results", []):
        if not result.get("passed", True):
            issues.append(f"[{result['severity']}] {result['check']}: {result['message']}")
    ...
    return {
        "data_quality": {
            ...
            "warnings": warnings,
            "issues": issues,
            # Flags for Council decision
            "has_gaps": any("time_gaps" in r.get("check", "") and not r.get("passed", True) 
                          for r in validation_summary.get("results", [])),
            "has_duplicates": any("duplicate" in r.get("check", "") and not r.get("passed", True) 
                                 for r in validation_summary.get("results", [])),
            "ohlc_valid": any("ohlc" in r.get("check", "") and r.get("passed", True) 
                            for r in validation_summary.get("results", [])),
            "volume_valid": any("volume" in r.get("check", "") and r.get("passed", True) 
                              for r in validation_summary.get("results", []))
        }
    }
