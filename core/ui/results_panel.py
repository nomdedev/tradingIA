...
class ResultsPanel(QWidget):
    ...
    def update_summary_tab(self):
        ...
        best_result = max(self.current_results, key=lambda r: r.metrics.get("sharpe_ratio", 0))
...
Risk-Adjusted Performance: {'Excellent' if sharpe > 2 and max_dd < 0.15 else 'Good' if sharpe > 1.5 else 'Needs Improvement'}
    ...
    def update_trades_tab(self):
        ...
        if trades:
            ...
            profit_factor = (
                abs(sum(t.get("pnl", 0) for t in winning_trades) / sum(t.get("pnl", 0) for t in losing_trades))
                if losing_trades
                else float("inf")
            )
...
• Largest Loss: ${min((t.get('pnl', 0) for t in losing_trades), default=0):.2f}
    ...
    def update_statistics_tab(self):
        ...
        stat_tests = [
            (
                "Sharpe Ratio t-test",
                f"{metrics.get('sharpe_t_stat', 0):.3f}",
                f"{metrics.get('sharpe_p_value', 0):.3f}",
            ),
            (
                "Normality Test (Shapiro)",
                f"{metrics.get('shapiro_stat', 0):.3f}",
                f"{metrics.get('shapiro_p_value', 0):.3f}",
            ),
...
accounting for the finite sample size and potential non-stationarity in returns.
    ...
    def export_report(self):
        ...
        filename, _ = QFileDialog.getSaveFileName(self, "Export Report", "", "HTML Files (*.html);;PDF Files (*.pdf)")
    ...
    def export_trades(self):
        ...
        filename, _ = QFileDialog.getSaveFileName(self, "Export Trades", "", "CSV Files (*.csv);;Excel Files (*.xlsx)")
