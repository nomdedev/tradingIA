...
class TradingAPI:
    ...
    async def run_backtest(self, request: BacktestRequest, background_tasks=None) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"task_id": task_id, "status": "running", "message": "Backtest started successfully"}
    ...
    def get_backtest_status(self, task_id: str) -> Dict[str, Any]:
        ...
        if task_id in self.running_tasks:
            return {"task_id": task_id, "status": "running", "message": "Backtest is still running"}
            ...
            return {"task_id": task_id, "status": "not_found", "message": "Backtest task not found"}
    ...
    async def add_broker(self, config: BrokerConfigRequest) -> Dict[str, Any]:
        ...
        try:
            ...
            broker_config = BrokerConfig(
                name=config.name, broker_type=config.broker_type, credentials=config.credentials, enabled=config.enabled
            )
