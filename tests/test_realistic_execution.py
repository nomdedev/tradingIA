...
def test_market_impact():
    ...
    impact_large = model.calculate_impact(
        order_size=5.0, price=50000, avg_volume=10, volatility=0.02, bid_ask_spread=50, time_of_day=14  # 5 BTC
    )
    ...
    optimal = model.estimate_optimal_order_size(
        available_capital=100000, price=50000, avg_volume=10, volatility=0.02, max_impact_pct=0.005
    )
...
def test_order_manager():
    ...
    market_order = manager.create_order(symbol="BTC-USD", side=OrderSide.BUY, order_type=OrderType.MARKET, quantity=0.1)
    ...
    filled = manager.process_orders(
        current_price=50000, current_timestamp=datetime.now(), high=50100, low=49900, volume=100, avg_volume=100
    )
    ...
    limit_order = manager.create_order(
        symbol="BTC-USD", side=OrderSide.BUY, order_type=OrderType.LIMIT, quantity=0.2, price=49000
    )

    # Process with price above limit (shouldn't fill)
    manager.process_orders(
        current_price=50000, current_timestamp=datetime.now(), high=50100, low=49500, volume=100, avg_volume=100
    )
    ...
    manager.process_orders(
        current_price=48900, current_timestamp=datetime.now(), high=49100, low=48800, volume=100, avg_volume=100
    )
    ...
    stop_order = manager.create_order(
        symbol="BTC-USD", side=OrderSide.SELL, order_type=OrderType.STOP_MARKET, quantity=0.1, stop_price=48000
    )

    # Process with price above stop (shouldn't trigger)
    manager.process_orders(
        current_price=49000, current_timestamp=datetime.now(), high=49500, low=48500, volume=100, avg_volume=100
    )
    ...
    manager.process_orders(
        current_price=47500, current_timestamp=datetime.now(), high=48100, low=47000, volume=100, avg_volume=100
    )
...
def test_integration():
    ...
    optimal = impact_model.estimate_optimal_order_size(
        available_capital=available_capital, price=price, avg_volume=avg_volume, volatility=0.02, max_impact_pct=0.005
    )
    print(f"   Optimal size: {optimal['optimal_size']:.4f} BTC")
    print(f"   Expected impact: {optimal['expected_impact_pct']:.4%}")
    ...
    exec_price_with_impact = impact_model.calculate_execution_price(
        side="buy", price=price, impact_pct=impact["total_impact_pct"]
    )
    ...
    latency_ms = latency_model.calculate_total_latency(order_type="market", market_volatility=1.0, time_of_day=14)
    ...
    order = order_manager.create_order(
        symbol="BTC-USD", side=OrderSide.BUY, order_type=OrderType.MARKET, quantity=optimal["optimal_size"]
    )
