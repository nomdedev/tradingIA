...
class TestBaseStrategy:
    ...
    def test_strategy_config_validation(self):
        ...
        # Valid config
        config = StrategyConfig(name="TestStrategy", description="Test strategy", parameters={"param1": 1})
    ...
    def test_abstract_methods(self):
        ...
        config = StrategyConfig(name="TestStrategy", description="Test strategy")
...
class TestMomentumStrategy:
    ...
    def test_calculate_indicators(self):
        ...
        data = pd.DataFrame(
            {
                "open": np.random.standard_normal(100) + 100,
                "high": np.random.standard_normal(100) + 102,
                "low": np.random.standard_normal(100) + 98,
                "close": np.random.standard_normal(100) + 100,
            },
            index=dates,
        )
    ...
    def test_generate_signals(self):
        ...
        data = pd.DataFrame({"open": prices - 1, "high": prices + 1, "low": prices - 1, "close": prices}, index=dates)

        config = StrategyConfig(
            name="MomentumStrategy",
            description="Test momentum strategy",
            parameters={"fast_period": 5, "slow_period": 10, "momentum_threshold": 0.01},
        )
...
class TestMeanReversionStrategy:
    ...
    def test_calculate_indicators(self):
        ...
        data = pd.DataFrame(
            {"open": prices - 0.5, "high": prices + 0.5, "low": prices - 0.5, "close": prices}, index=dates
        )
...
class TestBreakoutStrategy:
    ...
    def test_initialization(self):
        ...
        config = StrategyConfig(
            ...
            parameters={"lookback_period": 20, "breakout_threshold": 0.05, "consolidation_period": 10},
        )
