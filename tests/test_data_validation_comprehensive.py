...
class TestOHLCValidation:
    ...
    def test_invalid_high_low_relationship(self):
        ...
        invalid_data = pd.DataFrame(
            {"Open": [100], "High": [95], "Low": [105], "Close": [103], "Volume": [1000]}  # High < Low ❌
        )
    ...
    def test_invalid_close_above_high(self):
        ...
        invalid_data = pd.DataFrame(
            {"Open": [100], "High": [105], "Low": [95], "Close": [110], "Volume": [1000]}  # Close > High ❌
        )
    ...
    def test_invalid_close_below_low(self):
        ...
        invalid_data = pd.DataFrame(
            {"Open": [100], "High": [105], "Low": [95], "Close": [90], "Volume": [1000]}  # Close < Low ❌
        )
...
class TestDataGapsHandling:
    ...
    def test_detect_small_gaps(self):
        ...
        data = pd.DataFrame({"Date": dates, "Close": [100] * 7})
    ...
    def test_handle_gaps_forward_fill(self):
        ...
        data = pd.DataFrame({"Date": dates, "Close": [100, 101, 102, 103, 104, 105, 106, 107]}).set_index("Date")
    ...
    def test_raise_error_on_large_gaps(self):
        ...
        data = pd.DataFrame({"Date": dates, "Close": [100] * len(dates)})
    ...
    def test_weekend_gaps_ignored(self):
        ...
        data = pd.DataFrame({"Date": dates, "Close": [100] * 20})
...
class TestTimezoneHandling:
    ...
    def test_convert_to_utc(self):
        ...
        # Datos en EST
        dates_est = pd.date_range("2024-01-01", periods=10, freq="1h", tz="America/New_York")

        data = pd.DataFrame({"Date": dates_est, "Close": [100] * 10})
    ...
    def test_detect_timezone_mismatch(self):
        ...
        data = pd.DataFrame({"Date": mixed_dates, "Close": [100] * 10})
    ...
    def test_naive_datetime_warning(self):
        ...
        data = pd.DataFrame({"Date": naive_dates, "Close": [100] * 10})
...
class TestFutureDataDetection:
    ...
    def test_detect_future_data(self):
        ...
        data = pd.DataFrame({"Date": dates, "Close": [100] * 11})
    ...
    def test_allow_recent_data_with_tolerance(self):
        ...
        dates = pd.date_range(pd.Timestamp.now() - timedelta(hours=1), periods=10, freq="5min")

        data = pd.DataFrame({"Date": dates, "Close": [100] * 10})
...
class TestVolumeValidation:
    ...
    def test_zero_volume_detection(self):
        ...
        data = pd.DataFrame({"Close": [100, 101, 102], "Volume": [1000, 0, 1200]})  # Volumen 0 en medio
    ...
    def test_negative_volume_error(self):
        ...
        data = pd.DataFrame({"Close": [100, 101], "Volume": [1000, -500]})  # Volumen negativo ❌
    ...
    def test_handle_zero_volume_interpolation(self):
        ...
        data = pd.DataFrame({"Close": [100, 101, 102, 103], "Volume": [1000, 0, 0, 1200]})
...
class TestDuplicateTimestamps:
    ...
    def test_remove_duplicates_average(self):
        ...
        dates = pd.to_datetime(["2024-01-01 10:00", "2024-01-01 10:00", "2024-01-01 10:05"])
        data = pd.DataFrame({"Date": dates, "Close": [100, 102, 105]})
...
class TestLargeDatasetHandling:
    ...
    def test_process_million_bars(self):
        ...
        data = pd.DataFrame(
            {
                "Date": dates,
                "Close": 45000 + np.random.standard_normal(1_000_000) * 1000,
    ...
    def test_chunked_indicator_calculation(self):
        ...
        large_data = pd.DataFrame({"Close": np.random.standard_normal(500_000) + 45000})
