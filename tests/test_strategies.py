...
def generate_sample_data(periods=200):
    ...
    close_prices = 40000 + np.cumsum(np.random.standard_normal(periods) * 100)

    df = pd.DataFrame(
        {
            "open": close_prices + np.random.standard_normal(periods) * 50,
            "high": close_prices + abs(np.random.standard_normal(periods) * 100),
            "low": close_prices - abs(np.random.standard_normal(periods) * 100),
            "close": close_prices,
