...
class TestBacktesterCore:
    ...
    def setup_method(self):
        ...
        self.test_data = pd.DataFrame(
            {
                "close": 100 + np.cumsum(np.random.standard_normal(1000) * 0.5),
                "volume": np.random.randint(1000, 10000, 1000),
                "high": 100 + np.cumsum(np.random.standard_normal(1000) * 0.5) + 1,
                "low": 100 + np.cumsum(np.random.standard_normal(1000) * 0.5) - 1,
                "open": 100 + np.cumsum(np.random.standard_normal(1000) * 0.5) + 0.1,
            },
            index=dates,
        )
    ...
    def test_run_backtest_basic(self):
        ...
        # Ejecutar backtest
        results = self.backtester.run_backtest(DummyStrategy, self.test_data, {})
    ...
    def test_backtest_metrics_calculation(self):
        ...
        results = self.backtester.run_backtest(ConstantReturnStrategy, self.test_data, {})
