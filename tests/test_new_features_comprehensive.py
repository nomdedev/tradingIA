...
class TestCheckDataStatusScript:
    ...
    def test_check_data_status_function(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            try:
                ...
                result = check_data_status()
                # Verify function executed (returns None is OK, no exception is the key)
                assert result is None or isinstance(result, dict), "check_data_status should return None or dict"
            finally:
                os.chdir(old_cwd)
    ...
    def test_missing_files_handling(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            try:
                os.chdir(temp_dir)
                # Should handle missing files gracefully without raising
                result = check_data_status()
                assert result is None or isinstance(result, dict), "Should handle missing files gracefully"
            finally:
                os.chdir(old_cwd)
    ...
    def test_corrupted_csv_handling(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            try:
                os.chdir(temp_dir)
                # Should handle corrupted files gracefully without raising
                result = check_data_status()
                assert result is None or isinstance(result, dict), "Should handle corrupted CSV gracefully"
            finally:
                os.chdir(old_cwd)
...
class TestEdgeCasesAndErrorScenarios:
    ...
    def test_download_thread_with_invalid_timeframe(self):
        ...
        with patch("subprocess.Popen") as mock_popen:
            ...
            thread.download_finished.emit.assert_called_with(
                False, "Failed to download InvalidTimeframe data: Invalid timeframe specified"
            )
    ...
    def test_check_data_status_with_permission_denied(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            try:
                ...
                result = check_data_status()
                assert result is None or isinstance(result, dict), "Should handle permission errors gracefully"
            finally:
                os.chdir(old_cwd)
