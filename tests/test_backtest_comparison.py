...
class SimpleStrategy:
    ...
    def generate_signals(self, df_multi_tf):
        ...
        return {"entries": df["entries"], "exits": df["exits"], "signals": df[["signal", "entries", "exits"]]}
...
def run_comparison():
    ...
    backtester_simple = BacktesterCore(
        initial_capital=10000, commission=0.001, slippage_pct=0.001, enable_realistic_execution=False
    )

    results_simple = backtester_simple.run_simple_backtest(
        df_multi_tf=df_multi_tf, strategy_class=SimpleStrategy, strategy_params=strategy_params
    )
    ...
    sharpe_change = (
        ((metrics_realistic["sharpe"] - metrics_simple["sharpe"]) / abs(metrics_simple["sharpe"])) * 100
        if metrics_simple["sharpe"] != 0
        else 0
    )
    return_change = (
        ((metrics_realistic["total_return"] - metrics_simple["total_return"]) / abs(metrics_simple["total_return"]))
        * 100
        if metrics_simple["total_return"] != 0
        else 0
    )
    ...
    print(
        f"  Total Return: {return_change:+.1f}% ({metrics_simple['total_return']:.2%} → {metrics_realistic['total_return']:.2%})"
    )
