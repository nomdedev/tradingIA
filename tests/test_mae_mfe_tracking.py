...
def test_mae_mfe_tracking():
    ...
    expected_mae_short = (
        (max_price_short - entry_price_short) / entry_price_short if max_price_short > entry_price_short else 0.0
    )
    expected_mfe_short = (
        (entry_price_short - min_price_short) / entry_price_short if min_price_short < entry_price_short else 0.0
    )
    ...
    assert (
        abs(short_trade["mae"] - expected_mae_short) < 0.001
    ), f"Short MAE should be {expected_mae_short}, got {short_trade['mae']}"
    assert (
        abs(short_trade["mfe"] - expected_mfe_short) < 0.001
    ), f"Short MFE should be {expected_mfe_short}, got {short_trade['mfe']}"
    ...
    trades_records = pd.DataFrame({"pnl": [100, -50, 200, -100, 150]})
