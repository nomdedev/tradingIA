...
def sample_data():
    ...
    data = {
        "Date": dates,
        "Open": 45000 + np.random.standard_normal(100) * 1000,
        "High": 45500 + np.random.standard_normal(100) * 1000,
        "Low": 44500 + np.random.standard_normal(100) * 1000,
        "Close": 45000 + np.random.standard_normal(100) * 1000,
...
class TestStrategyEngine:
    ...
    def sample_config(self, temp_cache_dir):
        ...
        config = [
            ...
            },
            {
                ...
                "params": {"lookback": {"default": 100, "min": 50, "max": 200, "step": 10}},
            },
            {
                "name": "HFT_VMA",
                "module": "hft_momentum_vma",
                "class": "HFTVMAStrategy",
                "params": {"vma_period": {"default": 20, "min": 10, "max": 50, "step": 5}},
            },
            {
                "name": "LSTM_ML",
                "module": "lstm_ml_reversion",
                "class": "LSTMMLStrategy",
                "params": {"sequence_length": {"default": 60, "min": 30, "max": 120, "step": 10}},
            },
        ]
    ...
    def test_get_strategy_params(self, sample_config):
        ...
        assert math.isclose(atr_param["default"], 0.3)
        assert math.isclose(atr_param["min"], 0.1)
        assert math.isclose(atr_param["max"], 0.5)

    def test_validate_params_valid(self, sample_config):
        """Test parameter validation with valid parameters"""
        se = StrategyEngine(strategies_config_file=sample_config)

        valid_params = {"atr_multi": 0.3, "vol_thresh": 1.2}
    ...
    def test_validate_params_invalid_range(self, sample_config):
        ...
        invalid_params = {"atr_multi": 1.0, "vol_thresh": 1.2}  # Above max 0.5
