...
class TestOptimizedBacktester:
    ...
    def test_single_backtest(self):
        ...
        data = pd.DataFrame({"close": np.random.standard_normal(100).cumsum() + 100}, index=dates)
    ...
    def test_parallel_backtests(self):
        ...
        data = pd.DataFrame({"close": np.random.standard_normal(50).cumsum() + 100}, index=dates)

        backtester = OptimizedBacktester(max_workers=2)

        # Test parameter sets
        parameter_sets = [{"threshold": 0.01}, {"threshold": 0.02}, {"threshold": 0.005}]
