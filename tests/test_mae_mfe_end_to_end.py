...
def test_mae_mfe_end_to_end():
    ...
    backtester = BacktesterCore(
        initial_capital=10000, enable_kelly_position_sizing=True, enable_realistic_execution=True
    )
    ...
    results = backtester.run_simple_backtest(
        df_multi_tf=df_multi_tf, strategy_class=MockStrategy, strategy_params=strategy.get_params()
    )
