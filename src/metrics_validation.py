...
class MetricsValidator:
    ...
    def calculate_core_metrics(self, trades_df: pd.DataFrame) -> Dict[str, float]:
        ...
        if len(returns) > 1:
            sharpe_ratio = (np.mean(excess_returns) / np.std(excess_returns)) * np.sqrt(self.trading_days)
        else:
            sharpe_ratio = 0.0
        ...
        if len(downside_returns) > 0:
            sortino_ratio = (np.mean(returns) / np.std(downside_returns)) * np.sqrt(self.trading_days)
        else:
        ...
        information_ratio = (np.mean(returns) - btc_proxy_return) / tracking_error if tracking_error > 0 else 0.0
    ...
    def calculate_metrics_by_cluster(
        self, trades_df: pd.DataFrame, cluster_col: str = "confluence_score"
    ...
    def run_walk_forward_validation(
        ...
        """
        results = {"in_sample_metrics": [], "out_of_sample_metrics": [], "sharpe_oos": [], "periods": []}
    ...
    def run_monte_carlo_simulation(
        self, trades_df: pd.DataFrame, n_simulations: int = 500, noise_level: float = 0.1
    ) -> Dict[str, Any]:
        """
    ...
    def run_sensitivity_analysis(
        self, strategy_func: callable, param_ranges: Dict[str, List[float]], base_data: pd.DataFrame
    ) -> Dict[str, Any]:
        """
        ...
        if len(param_names) == 2:
            ...
            for param_name, param_values in param_ranges.items():
                ...
                results[param_name] = {"values": param_values, "sharpe_ratios": sharpe_values}

        return results

    def compare_systems(
        self,
        system_a_trades: pd.DataFrame,
        system_b_trades: pd.DataFrame,
        system_c_trades: Optional[pd.DataFrame] = None,
    ) -> Dict[str, Any]:
        """
    ...
    def generate_validation_report(
        self, metrics: Dict[str, Any], output_path: str = "results/validation_report.csv"
    ) -> None:
        """
    ...
    def plot_validation_results(
        self, wf_results: Dict[str, Any], mc_results: Dict[str, Any], output_dir: str = "results/figures"
    ) -> None:
        """
        ...
        if "sharpe_distribution" in mc_results and mc_results["sharpe_distribution"]:
            ...
            plt.axvline(
                x=np.mean(mc_results["sharpe_distribution"]),
                color="r",
                linestyle="--",
                linewidth=2,
...
def run_walk_forward_validation(
    df_5m: pd.DataFrame, strategy_func: callable, window_size: int = 8, test_periods: int = 3
) -> Dict[str, Any]:
...
def compare_systems(
    system_a_trades: pd.DataFrame, system_b_trades: pd.DataFrame, system_c_trades: Optional[pd.DataFrame] = None
) -> Dict[str, Any]:
