...
DEFAULT_COSTS_CONFIG = {
    ...
    "commission": {"maker_bps": 0, "taker_bps": 5},  # 0% para maker  # 0.05% para taker
    "funding": {"rate_bps": 1, "interval_hours": 8},  # 0.01% cada 8 horas
    "tax_drag": {"rate": 0.30, "apply_to_profits_only": True},  # 30% en ganancias
    "obi_filter": {"threshold": 0.2, "lookback_periods": 10},  # OBI > 0.2 para +1 score
}
...
class MicrostructureCosts:
    ...
    def calculate_obi(self, orderbook_data: pd.DataFrame, lookback: int = 10) -> pd.Series:
        """
        ...
        if "bid_volume" not in orderbook_data.columns or "ask_volume" not in orderbook_data.columns:
            ...
            obi_values = (bid_vol - ask_vol) / (bid_vol + ask_vol + 1e-8)  # Evitar división por cero
    ...
    def calculate_realistic_slippage(self, df: pd.DataFrame, position_size: float = 1.0) -> pd.Series:
        """
    ...
    def calculate_funding_costs(self, df: pd.DataFrame, position_hours: float) -> float:
        """
    ...
    def apply_all_costs(self, trades_df: pd.DataFrame, df_market: pd.DataFrame) -> pd.DataFrame:
        """
    ...
    def get_costs_summary(self, trades_df: pd.DataFrame, df_market: pd.DataFrame) -> Dict:
        """
        ...
        if len(trades_df) > 1:
            ...
            sharpe_before = (
                (excess_before.mean() / excess_before.std()) * np.sqrt(252) if excess_before.std() > 0 else 0.0
            )
        ...
        return {
            ...
            "cost_ratio": (
                abs(total_return_before - total_return_after) / abs(total_return_before)
                if total_return_before != 0
                else 0
            ),
        }
...
def integrate_microstructure_to_indicators(
    df: pd.DataFrame, orderbook_data: Optional[pd.DataFrame] = None
) -> pd.DataFrame:
    """
...
def realistic_costs_simulation(df: pd.DataFrame, trades_df: pd.DataFrame) -> Tuple[pd.DataFrame, Dict]:
    """
