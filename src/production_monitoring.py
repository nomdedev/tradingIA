...
class ProductionMonitor:
    ...
    def __init__(self, config_path: Optional[Path] = None, log_level: str = "INFO"):
        ...
        # Estado del sistema
        self.system_health = {"status": "initializing", "last_check": datetime.now(), "components": {}}

        # Alertas activas
        self.active_alerts = set()

        # Callbacks de alerta
        self.alert_callbacks: Dict[str, List[Callable]] = {"email": [], "sms": [], "webhook": [], "log": []}
    ...
    def _get_default_alerts_config(self) -> Dict:
        ...
        return {
            ...
            "errors": {"error_rate_threshold": 0.05, "max_consecutive_errors": 5, "api_timeout_threshold": 30},
            "data_drift": {
    ...
    def _setup_logging(self, log_level: str):
        ...
        fh = RotatingFileHandler(log_dir / "trading_monitor.log", maxBytes=10 * 1024 * 1024, backupCount=5)  # 10MB
    ...
    def stop_monitoring(self, timeout: float = 5.0):
        ...
        Args:
            timeout: Tiempo máximo de espera para que termine el thread (default 5s)
        """
        ...
        if self.monitor_thread and self.monitor_thread.is_alive():
            ...
            if self.monitor_thread.is_alive():
                self.logger.warning(f"Monitor thread did not terminate within {timeout}s")
    ...
    def _monitoring_loop(self, interval: int):
        ...
        while self.monitoring_active and not self._stop_event.is_set():
            try:
                ...
                self._check_alerts()
                # Usar wait con timeout para responder rápido al stop
    ...
    def _perform_health_check(self):
        ...
        health_data = {"timestamp": timestamp, "components": {}}
        ...
        self.system_health.update({"last_check": timestamp, "components": health_data["components"]})
    ...
    def _check_trading_health(self) -> Dict:
        ...
        return {"status": "healthy", "last_trade": None, "active_positions": 0, "pending_orders": 0}

    def _check_data_health(self) -> Dict:
        """Check salud del pipeline de datos"""
        # Implementar checks de calidad de datos
        return {"status": "healthy", "last_update": datetime.now(), "data_quality_score": 0.95}

    def _check_model_health(self) -> Dict:
        """Check salud del modelo"""
        # Implementar checks de modelo
        return {"status": "healthy", "last_retrain": None, "prediction_accuracy": 0.85}

    def record_performance_metric(self, metric_name: str, value: float, timestamp: Optional[datetime] = None):
        """
        ...
        metric_data = {"timestamp": timestamp, "metric": metric_name, "value": value}

        self.metrics_history["performance"].append(metric_data)

    def record_error(
        self, error_type: str, error_message: str, severity: str = "medium", context: Optional[Dict] = None
    ):
        """
    ...
    def record_latency(self, operation: str, latency_ms: float):
        ...
        """
        latency_data = {"timestamp": datetime.now(), "operation": operation, "latency_ms": latency_ms}
    ...
    def _check_performance_alerts(self):
        ...
        for metric_data in recent_perf:
            ...
            if metric_name == "sharpe" and value < self.alerts_config["performance"]["sharpe_threshold"]:
                self._trigger_alert(alert_key, f"Sharpe ratio below threshold: {value:.2f}", "warning")
            elif metric_name == "max_dd" and value > self.alerts_config["performance"]["max_dd_threshold"]:
                self._trigger_alert(alert_key, f"Max drawdown above threshold: {value:.2%}", "critical")
            elif metric_name == "win_rate" and value < self.alerts_config["performance"]["win_rate_threshold"]:
                self._trigger_alert(alert_key, f"Win rate below threshold: {value:.2%}", "warning")
    ...
    def _check_system_alerts(self):
        ...
        if PSUTIL_AVAILABLE:
            ...
            if cpu_percent > self.alerts_config["system"]["cpu_threshold"]:
                self._trigger_alert("system_cpu", f"CPU usage above threshold: {cpu_percent:.1f}%", "warning")

            if memory_percent > self.alerts_config["system"]["memory_threshold"]:
                self._trigger_alert("system_memory", f"Memory usage above threshold: {memory_percent:.1f}%", "critical")

            if disk_percent > self.alerts_config["system"]["disk_threshold"]:
                self._trigger_alert("system_disk", f"Disk usage above threshold: {disk_percent:.1f}%", "warning")
    ...
    def _check_error_alerts(self):
        ...
        recent_errors = [e for e in self.metrics_history["errors"] if e["timestamp"] > one_hour_ago]
        ...
        if recent_errors:
            ...
            if error_rate > self.alerts_config["errors"]["error_rate_threshold"]:
                self._trigger_alert(
                    "error_rate", f"Error rate above threshold: {error_rate:.3f} errors/min", "critical"
                )
            ...
            if consecutive_errors >= self.alerts_config["errors"]["max_consecutive_errors"]:
                self._trigger_alert(
                    "consecutive_errors", f"Consecutive high/critical errors: {consecutive_errors}", "critical"
                )
    ...
    def _trigger_alert(self, alert_key: str, message: str, severity: str):
        ...
        alert_data = {"timestamp": datetime.now(), "key": alert_key, "message": message, "severity": severity}
    ...
    def _get_metrics_summary(self) -> Dict:
        ...
        if self.metrics_history["performance"]:
            ...
            summary["performance"] = {"count": len(perf_data), "latest": perf_data[-1] if perf_data else None}
...
if __name__ == "__main__":
    ...
    logger.info("Monitoreo de Producción - Trading BTC Intradía")
    logger.info("=" * 60)
    logger.info("Características implementadas:")
    logger.info("• Monitoreo en tiempo real de sistema y performance")
    logger.info("• Alertas configurables (email/webhook/SMS)")
    logger.info("• Detección automática de degradación")
    logger.info("• Health checks completos")
    logger.info("• Dashboard con métricas históricas")
    logger.info("• Logging estructurado con rotación")
    logger.info("")
    logger.info("Métricas monitoreadas:")
    logger.info("• Performance: Sharpe, max DD, win rate")
    logger.info("• Sistema: CPU, memoria, disco")
    logger.info("• Errores: Tasa de error, errores consecutivos")
    logger.info("• Latencia: Operaciones críticas")
    logger.info("")
    logger.info("Configuración guardada en: config/monitoring_config.json")
    logger.info("Logs guardados en: logs/monitoring/")
