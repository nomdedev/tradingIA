...
class AnalysisEngines:
    ...
    def detect_regime_hmm(self, df_5m, n_states=3):
        ...
        try:
            ...
            regime_mapping = {0: "bear", 1: "chop", 2: "bull"}
    ...
    def granger_causality_test(self, signal_series, returns_series, max_lag=5):
        ...
        try:
            # Prepare data
            data = pd.DataFrame({"returns": returns_series, "signal": signal_series}).dropna()
    ...
    def placebo_test(self, trades_df, n_shuffles=100):
        ...
        try:
            ...
            for i in range(n_shuffles):
                ...
                shuffled_trades["timestamp"] = np.random.permutation(shuffled_trades["timestamp"].values)
            ...
            self.logger.info(f"Placebo test completed. Real Sharpe: {real_sharpe:.3f}, p-value: {p_value:.4f}")
            return p_value
    ...
    def calculate_good_vs_bad_entries(self, trades_df):
        ...
        try:
            ...
            if analysis["good_entries"]["win_rate"] > analysis["bad_entries"]["win_rate"]:
                analysis["recommendations"].append("Focus on high-score entries - they perform significantly better")
            else:
    ...
    def calculate_rr_metrics(self, trades_df):
        ...
        try:
            ...
            rr_segments = {"1:1": (0, 1), "1:2": (1, 2), "2:3": (2, 3), "3+": (3, float("inf"))}

            results = {}
            for segment_name, (min_rr, max_rr) in rr_segments.items():
                segment_trades = trades_df[(trades_df["rr_ratio"] >= min_rr) & (trades_df["rr_ratio"] < max_rr)]
    ...
    def _simulate_stress_backtest(self, stressed_df, scenario):
        ...
        try:
            ...
            if scenario == "flash_crash":
                ...
                return {"return_pct": 0.0, "max_dd_pct": 5.0, "survival": True, "description": "Unknown scenario"}

        except Exception as e:
            self.logger.error(f"Error simulating stress backtest: {e}")
            return {"return_pct": 0.0, "max_dd_pct": 0.0, "survival": True, "description": f"Error: {str(e)}"}
