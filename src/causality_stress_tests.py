...
class CausalityStressTester:
    ...
    def _save_config(self):
        ...
        config = {"stress_scenarios": self.stress_scenarios, "last_updated": pd.Timestamp.now().isoformat()}
        with open(self.config_path, "w") as f:
            json.dump(config, f, indent=2, default=str)

    def test_granger_causality(
        self, df: pd.DataFrame, indicators: List[str], target: str = "returns", max_lag: int = 5
    ) -> Dict:
        """
    ...
    def run_placebo_tests(
        self, df: pd.DataFrame, indicators: List[str], target: str = "returns", n_placebos: int = 100, max_lag: int = 5
    ) -> Dict:
        """
    ...
    def apply_stress_scenario(
        self, df: pd.DataFrame, scenario_name: str, start_idx: Optional[int] = None
    ) -> pd.DataFrame:
        """
        ...
        if scenario_name == "flash_crash":
            ...
            for i in range(duration_idx):
                idx = start_idx + i
                if idx < len(df_stressed):
                    noise = np.random.normal(0, df_stressed.loc[df_stressed.index[idx], "Close"] * 0.005 * vol_mult)
            ...
            for i in range(duration_idx):
                idx = start_idx + i
                if idx < len(df_stressed):
                    noise = np.random.normal(0, df_stressed.loc[df_stressed.index[idx], "Close"] * 0.001 * noise_mult)
                    df_stressed.loc[df_stressed.index[idx], "Close"] += noise

        return df_stressed

    def run_stress_test_suite(self, df: pd.DataFrame, strategy_func: callable, n_simulations: int = 50) -> Dict:
        """
    ...
    def generate_synthetic_data(
        self, df: pd.DataFrame, n_samples: int = 1000, noise_level: float = 0.1
    ) -> pd.DataFrame:
        """
        ...
        for _ in range(n_samples):
            ...
            for col in ["Close", "Open", "High", "Low"]:
                if col in synthetic_sample.columns:
                    noise = np.random.normal(0, synthetic_sample[col].std() * noise_level, len(synthetic_sample))
                    synthetic_sample[col] += noise

            # Ensure OHLC relationships
            for idx in synthetic_sample.index:
                row = synthetic_sample.loc[idx]
                synthetic_sample.loc[idx, "High"] = max(row[["Open", "Close", "High"]].max(), row["High"])
                synthetic_sample.loc[idx, "Low"] = min(row[["Open", "Close", "Low"]].min(), row["Low"])

            synthetic_data.append(synthetic_sample)

        return pd.concat(synthetic_data, keys=range(n_samples))

    def walk_forward_causality_validation(
        self,
        df: pd.DataFrame,
        indicators: List[str],
        target: str = "returns",
        ...
        """
        validation_results = {"causality_stability": [], "predictive_power": [], "placebo_consistency": []}
    ...
    def _generate_summary(self) -> Dict:
        ...
        summary["causality_rate"] = summary["causality_found"] / max(1, summary["total_indicators_tested"])
        summary["placebo_confirmation_rate"] = summary["placebo_confirmed"] / max(1, summary["causality_found"])

        return summary


def integrate_causality_stress_tests(df: pd.DataFrame, indicators: List[str] = None) -> Dict:
    """
    ...
    return {"causality": causality_results, "placebo": placebo_results, "summary": tester._generate_summary()}
