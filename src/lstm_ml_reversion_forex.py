...
def generate_synthetic_forex_data():
    """Generate synthetic EUR/USD forex data"""
    np.random.seed(42)
    n_bars = 1000  # Reduced data size

    # Base price around 1.10 (typical EUR/USD level)
    base_price = 1.10

    # Generate realistic price movements
    returns = np.random.normal(0, 0.001, n_bars)  # Daily returns ~0.1%
    prices = base_price * np.exp(np.cumsum(returns))

    # Generate OHLC from close prices
    highs = prices * (1 + np.abs(np.random.normal(0, 0.005, n_bars)))
    lows = prices * (1 - np.abs(np.random.normal(0, 0.005, n_bars)))
    opens = prices + np.random.normal(0, prices * 0.002, n_bars)
    opens = np.clip(opens, lows, highs)

    # Generate volume (forex volume is typically high)
    base_volume = 1000000  # 1M base volume
    volume = base_volume * (1 + np.random.exponential(0.5, n_bars))
...
class LSTMForexStrategy(Strategy):
    """
    LSTM ML Mean Reversion Strategy for Forex (EUR/USD)
...
class MeanReversionBaselineForex(Strategy):
    """Baseline mean reversion strategy for A/B testing"""
...
def run_lstm_forex_strategy():
    """Run LSTM Forex strategy with walk-forward testing and A/B analysis"""

    print("Running LSTM ML Mean Reversion Strategy for Forex (EUR/USD)")
    print("=" * 60)

    # Generate synthetic data
    data = generate_synthetic_forex_data()
    ...
    # Run backtest
    bt = Backtest(data, LSTMForexStrategy, cash=10000, commission=.0002)  # Forex commission
    result = bt.run()
    ...
    bt_baseline = Backtest(data, MeanReversionBaselineForex, cash=10000, commission=.0002)
    ...
    wf_results = perform_walk_forward_analysis(data, LSTMForexStrategy)
...
def save_results(lstm_result, baseline_result, wf_results):
    ...
    with open(results_dir / "lstm_forex_results.json", 'w') as f:
        json.dump(results, f, indent=4)

    print(f"\nResults saved to {results_dir}/lstm_forex_results.json")
...
def plot_results(bt, lstm_result, baseline_result):
    ...
    plt.subplot(2, 1, 1)
    bt.plot()
    plt.title('LSTM Forex Strategy - Equity Curve')
    ...
    plt.tight_layout()
    plt.savefig(plots_dir / "lstm_forex_analysis.png", dpi=300, bbox_inches='tight')
    plt.close()

    print(f"Plots saved to {plots_dir}/lstm_forex_analysis.png")


if __name__ == "__main__":
    run_lstm_forex_strategy()
