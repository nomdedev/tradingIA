...
class AlternativesIntegration:
    ...
    def _add_rsi_bb_signal(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        # Ensure close is numpy array for talib
        close_arr = df["close"].values.astype(float)
        volume_arr = df["volume"].values.astype(float)
        
        if TALIB_AVAILABLE and len(close_arr) >= 20:
            # RSI 14
            df["rsi_14"] = talib.RSI(close_arr, timeperiod=14)

            # Bollinger Bands
            bb_result = talib.BBANDS(close_arr, timeperiod=20, nbdevup=2, nbdevdn=2, matype=0)
            if len(bb_result) == 3 and len(bb_result[0]) > 0:
        ...
        df["rsi_bb_signal"] = (
            (df["rsi_14"] < 30) & (df["close"] < df["bb_lower"]) & (df["volume"] > 1.5 * df["vol_sma_20"])
        ).astype(int)
    ...
    def _add_vwap_delta_signal(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        df["vwap_delta_signal"] = (((df["vwap_cross_up"] | df["vwap_cross_down"])) & (df["vol_delta"] > 0.20)).astype(
            int
        )
    ...
    def _add_macd_adx_signal(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if TALIB_AVAILABLE and len(close_arr) >= 26:
            # MACD
            macd_result = talib.MACD(close_arr, fastperiod=12, slowperiod=26, signalperiod=9)
            if len(macd_result) == 3 and len(macd_result[2]) > 0:
                df["macd_hist"] = macd_result[2]
            else:
                df["macd_hist"] = self._calculate_macd_hist(df["close"])

            # ADX
            adx_result = talib.ADX(high_arr, low_arr, close_arr, timeperiod=14)
            if len(adx_result) > 0:
                df["adx"] = adx_result
            else:
        ...
        df["macd_adx_signal"] = ((df["macd_hist"] > 0) & (df["adx"] > 25)).astype(int)
    ...
    def _add_ichimoku_aroon_signal(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        # Aroon Up
        if TALIB_AVAILABLE and len(high_arr) >= 14:
            aroon_result = talib.AROON(high_arr, low_arr, timeperiod=14)
            if len(aroon_result) == 2 and len(aroon_result[0]) > 0:
                df["aroon_up"] = aroon_result[0]
            else:
        ...
        df["ichimoku_aroon_signal"] = (df["cloud_break_up"] & (df["aroon_up"] > 70)).astype(int)
    ...
    def _add_stochastic_funding_signal(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if TALIB_AVAILABLE and len(close_arr) >= 14:
            stoch_result = talib.STOCH(
                high_arr, low_arr, close_arr, fastk_period=14, slowk_period=3, slowd_period=3
            )
            if len(stoch_result) == 2 and len(stoch_result[0]) > 0:
        ...
        df["stochastic_funding_signal"] = (
            (df["stoch_k"] < 20) & (df["stoch_d"] < 20) & (df["funding_rate"] > 0.005)  # Funding rate > 0.05%
        ).astype(int)
    ...
    def _generate_hybrid_signal(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if "fvg_signal" in df.columns:
            ...
            df["hybrid_signal"] = (
                (df["fvg_signal"] == 1) & (df["alternative_score_norm"] >= 3) & (df.get("vol_delta", 0) > 0.20)
            ).astype(int)
        else:
    ...
    def _calculate_aroon_up(self, high: pd.Series, low: pd.Series, period: int = 14) -> pd.Series:
        ...
        def days_since_high(window):
            if len(window) < period:
                return np.nan
            idx_max = window.argmax()
            return period - 1 - idx_max
        
        days_high = high.rolling(window=period).apply(lambda x: period - 1 - x.argmax(), raw=True)
        aroon_up = 100 * (1 - days_high / (period - 1))
        return aroon_up
    ...
    def backtest_alternatives(self, df_signals: pd.DataFrame, initial_capital: float = 10000) -> Dict:
        """
...
def main():
    ...
    df = pd.DataFrame(
        {
            "open": 45000 + np.random.standard_normal(1000).cumsum() * 100,
            "high": 45000 + np.random.standard_normal(1000).cumsum() * 100 + 200,
            "low": 45000 + np.random.standard_normal(1000).cumsum() * 100 - 200,
            "close": 45000 + np.random.standard_normal(1000).cumsum() * 100,
