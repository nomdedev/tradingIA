...
class PairsTradingForexStrategy(Strategy):
    """
    Pairs Trading Cointegration Strategy for Forex (EUR/USD vs GBP/USD)
    Market-neutral strategy exploiting mean-reversion in cointegrated pairs
    """

    # Strategy parameters (forex-optimized)
    ...
    max_holding_period = 20  # Max bars to hold position (forex 1h = 20 hours)

    def init(self):
        # Initialize spread tracking (simplified for single pair)
        close = self.data.Close
        high = self.data.High
        low = self.data.Low

        # Create synthetic spread using EUR/USD with some noise to simulate GBP/USD relationship
        # In practice, this would use actual EUR/USD and GBP/USD data
        np.random.seed(42)  # For reproducible synthetic spread
        spread_noise = np.random.normal(0, 0.001, len(close))  # Small noise
        self.spread = close + spread_noise  # Synthetic spread

        # Calculate spread statistics
        spread_series = pd.Series(self.spread)
        self.spread_mean = spread_series.rolling(window=100).mean().values
        self.spread_std = spread_series.rolling(window=100).std().values

        # Z-score of spread
        self.spread_zscore = ((spread_series - spread_series.rolling(window=100).mean()) /
                              spread_series.rolling(window=100).std()).values

        # Higher timeframe trend bias (4h equivalent for 1h data = 4 periods)
        htf_close = pd.Series(close).rolling(window=4).mean()
        ...
        # Position tracking
        self.entry_bar = 0
        self.position_type = 0  # 0=none, 1=long spread, -1=short spread

    def next(self):
        # Ensure we have enough data
        if len(self.data.Close) < 150:  # Need at least 150 periods for spread stats
            return
        ...
        # Entry conditions for LONG spread (EUR underperforms GBP)
        ...
        if spread_wide and trend_bias and not_reversing and not self.position:
            ...
            stop_distance = self.spread_std[idx] * self.risk_mult
            if stop_distance > 0:
                position_size = min(
            ...
            # Entry with SL/TP based on spread
            # For long orders: TP above entry, SL below entry
            sl_level = self.spread[idx] - stop_distance  # SL below entry
            tp_level = self.spread[idx] + (stop_distance * self.tp_rr)  # TP above entry

            # For backtesting, ensure proper order: for long orders TP > entry > SL
            if tp_level > self.spread[idx] > sl_level:
                self.buy(size=size_fraction, sl=sl_level, tp=tp_level)
                self.entry_bar = idx
                self.position_type = 1

        # Entry conditions for SHORT spread (EUR outperforms GBP)
        ...
        if spread_wide_short and trend_bias_short and not_reversing_short and not self.position:
            ...
            sl_level = self.spread[idx] + stop_distance  # SL above entry
            tp_level = self.spread[idx] - (stop_distance * self.tp_rr)  # TP below entry
...
def calculate_cointegration(eur_data: pd.DataFrame,
                            gbp_data: pd.DataFrame) -> Tuple[bool, float, float, float]:
    """Calculate cointegration between EUR/USD and GBP/USD"""
    # Ensure same length
    min_len = min(len(eur_data), len(gbp_data))
    eur_close = eur_data['Close'].iloc[-min_len:]
    gbp_close = gbp_data['Close'].iloc[-min_len:]

    # Test cointegration
    try:
        coint_t, p_value, crit_values = coint(eur_close, gbp_close)
        cointegrated = p_value < 0.05  # 5% significance

        # Calculate hedge ratio
        model = OLS(eur_close, gbp_close).fit()
        hedge_ratio = model.params[0]

        # Calculate spread
        spread = eur_close - hedge_ratio * gbp_close
...
def walk_forward_test_forex(df: pd.DataFrame, params: Dict = None) -> Dict:
    """Walk-forward optimization and testing for forex pairs strategy"""
    if not BACKTESTING_AVAILABLE:
        print("Backtesting library not available")
        return {}

    if params:
        PairsTradingForexStrategy.spread_zscore_threshold = params.get(
            'spread_zscore_threshold', 2.0)
        PairsTradingForexStrategy.vol_mult = params.get('vol_mult', 1.2)
        PairsTradingForexStrategy.risk_mult = params.get('risk_mult', 1.5)
        PairsTradingForexStrategy.tp_rr = params.get('tp_rr', 2.0)

    # Simple train/test split (80/20) for forex
    ...
    try:
        bt = Backtest(test_data, PairsTradingForexStrategy,
                      cash=10000, commission=0.0002)  # Forex commission
        result = bt.run()
...
def optimize_forex_pairs_strategy(df: pd.DataFrame) -> Dict:
    """Grid search optimization for forex pairs strategy parameters"""
    if not BACKTESTING_AVAILABLE:
    ...
    print("Running parameter optimization for forex pairs...")

    # Parameter ranges optimized for forex pairs (reduced for speed)
    param_grid = {
    ...
    # Use 50% of data for optimization (faster)
        ...
        for vol_mult in param_grid['vol_mult']:
            for risk_mult in param_grid['risk_mult']:
                for tp_rr in param_grid['tp_rr']:
                    ...
                    # Set parameters
                    PairsTradingForexStrategy.spread_zscore_threshold = spread_thresh
                    PairsTradingForexStrategy.vol_mult = vol_mult
                    PairsTradingForexStrategy.risk_mult = risk_mult
                    PairsTradingForexStrategy.tp_rr = tp_rr

                    try:
                        # Quick backtest on training data
                        bt = Backtest(train_df, PairsTradingForexStrategy,
                        ...
                        if trades >= 20 and win_rate > 0.4 and sharpe > best_sharpe:
                            best_sharpe = sharpe
                            best_params = {
...
def ab_test_vs_mean_reversion(df: pd.DataFrame) -> Dict:
    """A/B test against Mean Reversion IBS+BB alternative for forex"""
    if not BACKTESTING_AVAILABLE:
        return {}
    ...
    class MeanReversionForexStrategy(Strategy):
        ...
        def next(self):
            ...
            vol_confirm = True  # Simplified for forex
    ...
    try:
        # Pairs strategy
        bt_pairs = Backtest(df, PairsTradingForexStrategy, cash=10000, commission=0.0002)
        result_pairs = bt_pairs.run()

        # Mean reversion strategy
        bt_mr = Backtest(df, MeanReversionForexStrategy, cash=10000, commission=0.0002)
        result_mr = bt_mr.run()
...
def run_forex_pairs_strategy_analysis(data_path: str = None):
    """Complete forex pairs trading strategy analysis"""
    print("=== Forex Pairs Trading Cointegration Strategy Analysis (EUR/USD vs GBP/USD) ===")
    ...
    if data_path:
        ...
        np.random.seed(42)
        dates = pd.date_range('2010-01-01', '2024-01-01', freq='H')

        # EUR/USD characteristics (major forex pair)
        base_price = 1.35
        returns = np.random.normal(0.00001, 0.0008, len(dates))  # Low vol for forex
        # Add some trend and seasonality
        trend = 0.000002 * np.sin(np.arange(len(dates)) * 0.00001)
        seasonal = 0.0001 * np.sin(2 * np.pi * np.arange(len(dates)) / (24 * 365))  # Annual cycle

        final_returns = returns + trend + seasonal
        prices = base_price * np.exp(np.cumsum(final_returns))

        # Create OHLCV data
        high_mult = 1 + np.random.exponential(0.0005, len(dates))
        low_mult = 1 - np.random.exponential(0.0005, len(dates))
    ...
    best_params = optimize_forex_pairs_strategy(df)
    ...
    bt = Backtest(df, PairsTradingForexStrategy, cash=10000, commission=0.0002)
    result = bt.run()
    ...
    # Save results
    output_dir = Path('results/forex_pairs')
    ...
    with open(output_dir / 'metrics_forex_pairs.json', 'w') as f:
        json.dump(results, f, indent=2, default=str)

    # Create visualization
    fig, axes = plt.subplots(2, 2, figsize=(15, 10))
    fig.suptitle(
        'Forex Pairs Trading Cointegration Strategy Analysis (EUR/USD vs GBP/USD)',
        fontsize=16)
    ...
    plt.tight_layout()
    plt.savefig(output_dir / 'forex_pairs_analysis.png', dpi=300, bbox_inches='tight')
    plt.close()

    print(f"Analysis complete. Results saved to {output_dir}/")
    print(f"""
    Final Results for EUR/USD-GBP/USD Pairs:
...
if __name__ == "__main__":
    # Run analysis for EUR/USD vs GBP/USD pairs
    run_forex_pairs_strategy_analysis()
