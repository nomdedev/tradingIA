...
class SessionLogger:
    ...
    def __init__(self, reports_dir="reports/sessions"):
        ...
        self.session_data = {
            ...
            "user_actions": [],
            "errors": [],
            "results": {"backtests_run": 0, "live_trading_sessions": 0, "data_downloads": 0, "strategies_tested": []},
    ...
    def log_data_download(self, timeframe, status, file_path=None, error=None):
        ...
        self.log_action(
            "data_download",
            {"timeframe": timeframe, "status": status, "file_path": file_path, "error": error},
            result=status,
        )
    ...
    def log_window_event(self, event_type, window_info):
        ...
        self.log_action("window_event", {"event_type": event_type, "window_info": window_info})
    ...
    def log_data_loading_event(self, event_type, data_info):
        ...
        self.log_action("data_loading", {"event_type": event_type, "data_info": data_info})
    ...
    def _create_text_report(self):
        ...
        try:
            with open(text_file, "w", encoding="utf-8") as f:
                ...
                backtests = [action for action in self.session_data["user_actions"] if action["type"] == "backtest"]
                if backtests:
                    f.write("-" * 80 + "\n")
                    ...
                    avg_sharpe = sum(
                        bt.get("details", {}).get("key_metrics", {}).get("sharpe_ratio", 0) for bt in backtests
                    ) / max(1, total_backtests)
                    ...
                    best_bt = max(
                        backtests, key=lambda x: x.get("details", {}).get("key_metrics", {}).get("sharpe_ratio", -999)
                    )
                ...
                data_actions = [
                    action for action in self.session_data["user_actions"] if "data" in action["type"].lower()
                ]
                if data_actions:
                    f.write("-" * 80 + "\n")
