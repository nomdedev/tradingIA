...
from config.mtf_config import ALPACA_CONFIG, MTF_CONFIG, TRADING_CONFIG, DATA_DIR
import pandas as pd
import numpy as np
...
class MultiTFDataHandler:
    ...
    def __init__(self):
        ...
        self.api = tradeapi.REST(ALPACA_CONFIG["api_key"], ALPACA_CONFIG["secret_key"], ALPACA_CONFIG["base_url"])
    ...
    def _download_timeframe(
        self, symbol: str, timeframe: str, start: str, end: str, max_retries: int = 3
        ...
        for attempt in range(max_retries):
            try:
                ...
                bars = self.api.get_crypto_bars(
                    symbol, timeframe=tf_obj, start=start_dt.isoformat(), end=end_dt.isoformat()
                ).df
    ...
    def add_multi_tf_filters(self, dfs: Dict[str, pd.DataFrame]) -> Dict[str, pd.DataFrame]:
        ...
        df_5m["bear_filter"] = ~df_5m["uptrend_1h"] & df_5m["vol_filter"]
    ...
    def add_noise(self, df: pd.DataFrame, vol_pct: float = 50, seed: int = None) -> pd.DataFrame:
        """
...
def example_usage():
    ...
    # Get multi-TF data
    dfs = handler.get_multi_tf_data(symbol="BTCUSD", start_date="2024-01-01", end_date="2024-12-31")
    ...
    print(f"  momentum_15m: {df_5m['momentum_15m'].sum()} bars ({df_5m['momentum_15m'].mean()*100:.1f}%)")
    print(f"  bull_filter: {df_5m['bull_filter'].sum()} bars ({df_5m['bull_filter'].mean()*100:.1f}%)")
    print(f"  bear_filter: {df_5m['bear_filter'].sum()} bars ({df_5m['bear_filter'].mean()*100:.1f}%)")
