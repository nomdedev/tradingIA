...
class ABTestThread(QThread):
    ...
    def run(self):
        try:
            ...
            # Run backtest A
            results_a = self.backtester.run_simple_backtest(self.data_dict, self.strategy_a, self.params_a)

            self.progress_updated.emit(50, "Running Strategy B backtest...")

            # Run backtest B
            results_b = self.backtester.run_simple_backtest(self.data_dict, self.strategy_b, self.params_b)
            ...
            results = {
                "strategy_a": {"name": str(self.strategy_a), "results": results_a},
                "strategy_b": {"name": str(self.strategy_b), "results": results_b},
    ...
    def generate_recommendation(self, comparison, results_a, results_b):
        ...
        if sharpe_comp["percent_diff"] != 0:
            reasons.append(
                f"Strategy {sharpe_comp['winner']} has " f"{abs(sharpe_comp['percent_diff']):.1f}% better Sharpe ratio"
            )

        if return_comp["percent_diff"] != 0:
            reasons.append(
                f"Strategy {return_comp['winner']} has " f"{abs(return_comp['percent_diff']):.1f}% higher returns"
            )
...
class Tab5ABTesting(QWidget):
    ...
    def create_selection_section(self):
        ...
        title.setStyleSheet("font-size: 15px; font-weight: 600; color: #cccccc;")
        ...
        a_header.setStyleSheet("font-weight: bold; color: #569cd6; font-size: 16px;")
        a_layout.addWidget(a_header)

        self.strategy_a_combo = QComboBox()
        self.strategy_a_combo.setMinimumHeight(45)
        ...
        b_header.setStyleSheet("font-weight: bold; color: #f48771; font-size: 16px;")
        b_layout.addWidget(b_header)

        self.strategy_b_combo = QComboBox()
        self.strategy_b_combo.setMinimumHeight(45)
        ...
        self.run_btn.setFixedSize(250, 55)
    ...
    def on_run_ab_test(self):
        ...
        self.test_thread = ABTestThread(
            self.backtester, strategy_a, strategy_b, self.parent_platform.data_dict, params_a, params_b
        )
    ...
    def display_results(self, results):
        ...
        self.confidence_text.setText(
            f"Confidence: {confidence} | Score: A={recommendation['score_a']} vs B={recommendation['score_b']}"
        )
