    ...
    def run(self):
        try:
            ...
            df = self.data_manager.load_alpaca_data(
                symbol=self.symbol, start_date=self.start_date, end_date=self.end_date, timeframe=self.timeframe
            )
...
class DataCard(QFrame):
    ...
    def __init__(self, timeframe, bars, date_range):
        ...
        tf_label.setStyleSheet(f"color: {DarkTheme.SUCCESS}; font-size: 11px; font-weight: 600;")

        # Bars count
        bars_label = QLabel(f"{bars:,} bars")
        bars_label.setStyleSheet(f"color: {DarkTheme.TEXT_PRIMARY}; font-size: 13px; font-weight: 700;")
        ...
        range_label.setStyleSheet(f"color: {DarkTheme.TEXT_SECONDARY}; font-size: 10px;")
...
class Tab1DataManagement(QWidget):
    ...
    def create_configuration_panel(self):
        ...
        widget = QWidget()
        widget.setMaximumWidth(450)  # Limit width for better layout on wide screens
        layout = QVBoxLayout()
        ...
        source_btn_layout.setSpacing(10)

        self.alpaca_btn = QPushButton("Alpaca")
        self.alpaca_btn.setCheckable(True)
        self.alpaca_btn.setChecked(True)
        self.alpaca_btn.setCursor(Qt.CursorShape.PointingHandCursor)
        self.alpaca_btn.setFixedHeight(35)
        ...
        binance_btn.setCursor(Qt.CursorShape.PointingHandCursor)
        binance_btn.setFixedHeight(35)
        ...
        self.multi_tf_check.setStyleSheet(
            """
            QCheckBox {
                font-size: 14px;
                color: #ccc;
        ...
        def create_label(text):
            lbl = QLabel(text)
            lbl.setStyleSheet(f"color: {DarkTheme.TEXT_PRIMARY}; font-size: 14px; font-weight: 500;")
            return lbl

        params_layout.addRow(create_label("Symbol:"), self.symbol_combo)
        params_layout.addRow(create_label("Timeframe:"), self.timeframe_combo)
        params_layout.addRow(create_label("Start Date:"), self.start_date_edit)
        params_layout.addRow(create_label("End Date:"), self.end_date_edit)
        ...
        self.load_data_btn.setFixedHeight(55)
    ...
    def on_load_data_clicked(self):
        ...
        if hasattr(self.parent_platform, "session_logger") and self.parent_platform.session_logger:
            self.parent_platform.session_logger.log_ui_event(
        ...
        self.load_thread = DataLoadThread(self.data_manager, symbol, timeframe, start_date, end_date, multi_tf)
    ...
    def on_data_loaded(self, data_dict):
        ...
        if hasattr(self.parent_platform, "session_logger") and self.parent_platform.session_logger:
            self.parent_platform.session_logger.log_data_loading_event(
                "manual_load_success",
                {"timeframes": list(data_dict.keys()), "total_bars": total_bars, "timeframes_count": len(data_dict)},
            )

        self.parent_platform.update_status(f"Loaded {len(data_dict)} timeframes ({total_bars:,} total bars)", "success")
    ...
    def on_error_occurred(self, error_message):
        ...
        if hasattr(self.parent_platform, "session_logger") and self.parent_platform.session_logger:
            self.parent_platform.session_logger.log_error(
                error_type="data_loading_error", error_message=error_message, context={"tab": "Tab1_DataManagement"}
            )
