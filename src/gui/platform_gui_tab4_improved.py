...
class Tab4ResultsAnalysis(QWidget):
    ...
    def create_left_panel(self):
        ...
        widget = QWidget()
        widget.setMaximumWidth(300)  # Limit width
        layout = QVBoxLayout()
        ...
        current_label.setStyleSheet("color: #4ec9b0; font-size: 14px;")
        current_layout.addWidget(current_label)

        self.current_result_text = QLabel("No backtest loaded")
        self.current_result_text.setWordWrap(True)
        self.current_result_text.setStyleSheet("color: #cccccc; font-size: 13px;")
        ...
        history_label.setStyleSheet("color: #cccccc; font-size: 14px; margin-top: 10px;")
        ...
        self.save_current_btn.setCursor(Qt.CursorShape.PointingHandCursor)
        self.save_current_btn.setFixedHeight(40)
        ...
        self.multiselect_check.setStyleSheet(
            """
            QCheckBox {
                color: #cccccc; 
                font-size: 13px;
    ...
    def create_right_panel(self):
        ...
        self.chart_type_combo.setFixedHeight(35)
        ...
        chart_label.setStyleSheet("color: #ccc; font-size: 14px; font-weight: 600;")
        ...
        self.chart_view.setMinimumHeight(500)  # Taller chart
        self.chart_view.setStyleSheet("background-color: #252525; border: 1px solid #3d3d3d; border-radius: 6px;")
        ...
        bottom_tabs.addTab(trades_tab, "Trades")

        # FASE 2: Risk Analysis Tab
        risk_tab = self.create_risk_analysis_tab()
        bottom_tabs.addTab(risk_tab, "Risk Analysis (MAE/MFE)")
    ...
    def create_risk_analysis_tab(self):
        """Create Risk Analysis tab with MAE/MFE visualization"""
        widget = QWidget()
        layout = QVBoxLayout()
        
        # Info header
        info_label = QLabel("Maximum Adverse Excursion (MAE) & Maximum Favorable Excursion (MFE) Analysis")
        info_label.setStyleSheet("color: #ccc; font-weight: bold; font-size: 14px; margin-bottom: 10px;")
        layout.addWidget(info_label)
        
        # Chart view for Risk Analysis
        self.risk_chart_view = QWebEngineView()
        self.risk_chart_view.setStyleSheet("background-color: #1e1e1e; border: 1px solid #3d3d3d; border-radius: 4px;")
        layout.addWidget(self.risk_chart_view)
    ...
    def create_trades_tab(self):
        ...
        self.trade_table.setHorizontalHeaderLabels(
            ["Timestamp", "Direction", "Entry", "Exit", "PnL%", "Score", "Duration", "Type", "Exit Reason", "MAE%"]
        )
    ...
    def load_results(self, results):
        ...
        try:
            ...
            # Update Risk Analysis (MAE/MFE)
            self.update_risk_analysis(results)
    ...
    def update_risk_analysis(self, results):
        """Update Risk Analysis tab with MAE/MFE charts"""
        try:
            trades = results.get("trades", [])
            if not trades:
                self.risk_chart_view.setHtml("<h3 style='color:#ccc; text-align:center'>No trades available for analysis</h3>")
                return
            ...
            if "mae" not in df_trades.columns or "mfe" not in df_trades.columns:
                self.risk_chart_view.setHtml("<h3 style='color:#ccc; text-align:center'>MAE/MFE data not available in trade records</h3>")
                return
                
            # Create subplots: Scatter MAE vs MFE, and Distributions
            fig = make_subplots(
                rows=2, cols=2,
                specs=[[{"colspan": 2}, None], [{}, {}]],
                subplot_titles=("MAE vs MFE Scatter (PnL Color)", "MAE Distribution", "MFE Distribution"),
                vertical_spacing=0.15
            )
            
            # Scatter Plot
            # Color by PnL (Green for win, Red for loss)
            colors = ['#4ec9b0' if pnl > 0 else '#f48771' for pnl in df_trades['pnl']]
            
            fig.add_trace(
                go.Scatter(
                    x=df_trades['mae'] * 100, # Convert to %
                    y=df_trades['mfe'] * 100,
                    mode='markers',
                    marker=dict(
                        color=colors,
                    ...
                    text=[f"PnL: ${pnl:.2f}<br>MAE: {mae:.2%}<br>MFE: {mfe:.2%}" 
                          for pnl, mae, mfe in zip(df_trades['pnl'], df_trades['mae'], df_trades['mfe'])],
                    hoverinfo='text',
                    name='Trades'
                ),
                row=1, col=1
            ...
            max_val = max(df_trades['mae'].max(), df_trades['mfe'].max()) * 100
            fig.add_trace(
                go.Scatter(
            ...
            fig.update_layout(
                ...
                height=600,
                showlegend=False,
                title_text="Risk Analysis: Trade Excursions"
            )
            ...
            fig.update_xaxes(title_text="MFE (%)", row=2, col=2)
            
            html = fig.to_html(include_plotlyjs="cdn")
            self.risk_chart_view.setHtml(html)
            
        except Exception as e:
            self.logger.error(f"Error updating risk analysis: {e}")
            self.risk_chart_view.setHtml(f"<h3 style='color:#f48771'>Error: {str(e)}</h3>")
    ...
    def render_equity_chart(self, results):
        ...
        try:
            ...
            if "equity_curve" in results:
                ...
                fig.add_trace(go.Scatter(y=equity, mode="lines", name="Current", line=dict(color="#4ec9b0", width=2)))
    ...
    def render_drawdown_chart(self, results):
        ...
        try:
            ...
            fig = make_subplots(rows=2, cols=1, row_heights=[0.6, 0.4], subplot_titles=("Equity Curve", "Drawdown %"))

            # Equity
            fig.add_trace(
                go.Scatter(y=equity, mode="lines", name="Equity", line=dict(color="#4ec9b0", width=2)), row=1, col=1
            )

            # Drawdown
            fig.add_trace(
                go.Scatter(
                    y=drawdown, mode="lines", name="Drawdown", fill="tozeroy", line=dict(color="#f48771", width=1)
                ),
                row=2,
                col=1,
            )
    ...
    def render_distribution_chart(self, results):
        ...
        try:
            ...
            if not wins.empty:
                fig.add_trace(go.Histogram(x=wins, name="Wins", marker_color="#4ec9b0", opacity=0.75, nbinsx=30))

            # Losers
            losses = trades_df[trades_df["pnl_pct"] <= 0]["pnl_pct"]
            if not losses.empty:
                fig.add_trace(go.Histogram(x=losses, name="Losses", marker_color="#f48771", opacity=0.75, nbinsx=30))
        ...
        try:
            ...
            fig.add_trace(
                go.Scatter(
                    ...
                    mode="markers",
                    marker=dict(size=sizes, color=colors, line=dict(width=1, color="#1e1e1e")),
                    text=[f"Trade {i+1}<br>PnL: {pnl:.2f}%" for i, pnl in enumerate(trades_df.get("pnl_pct", []))],
                    hoverinfo="text",
                )
            )
        ...
        try:
            ...
            fig.add_trace(
                go.Indicator(
                    ...
                    domain={"x": [0, 1], "y": [0, 1]},
                    gauge={"axis": {"range": [0, 50]}, "bar": {"color": "#f48771"}},
                ),
                row=1,
                col=2,
            )
            ...
            fig.add_trace(
                go.Indicator(
                    ...
                    domain={"x": [0, 1], "y": [0, 1]},
                    gauge={"axis": {"range": [0, 100]}, "bar": {"color": "#569cd6"}},
                ),
                row=1,
                col=3,
            )

            fig.update_layout(template="plotly_dark", height=300, paper_bgcolor="#1e1e1e", font=dict(color="#cccccc"))
    ...
    def save_current_backtest(self):
        ...
        if ok and name:
            try:
                ...
                data = {"name": name, "timestamp": timestamp, "results": self.current_results}
    ...
    def on_trade_double_clicked(self, item):
        ...
        QMessageBox.information(
            self, f"Trade {row + 1} Details", "\n".join([f"{k}: {v}" for k, v in trade_data.items()])
        )

    def clear_display(self):
        """Clear all displays"""
        self.chart_view.setHtml(
            "<div style='color:#cccccc; padding:20px;'><h3>No data loaded</h3><p>Run a backtest to see results</p></div>"
        )
        self.trade_table.setRowCount(0)
        self.good_stats_table.setRowCount(0)
        self.bad_stats_table.setRowCount(0)
