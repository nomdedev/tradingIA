...
class Tab2StrategyConfig(QWidget):
    ...
    def create_styled_label(self, text, color="#cccccc", font_size="14px", bold=False):
        """Create a styled QLabel"""
        print(f"DEBUG: Creating styled label: {text}")
        label = QLabel(text)
        weight = "bold" if bold else "normal"
        label.setStyleSheet(f"color: {color}; font-size: {font_size}; font-weight: {weight};")
        return label
    ...
    def create_mode_selector(self):
        ...
        frame.setStyleSheet(
            ...
            QRadioButton::indicator::checked {{
                ...
                border: 2px solid {DarkTheme.ACCENT_PRIMARY};
                border-radius: 10px;
            }}
        ...
        self.research_radio.setCursor(Qt.CursorShape.PointingHandCursor)
        ...
        self.production_radio.setCursor(Qt.CursorShape.PointingHandCursor)
        ...
        self.mode_description.setStyleSheet("color: #cccccc; font-size: 14px; padding: 5px 0;")
    ...
    def create_configuration_panel(self):
        ...
        widget = QWidget()
        widget.setMaximumWidth(450)  # Limit width for better layout
        layout = QVBoxLayout()
        ...
        self.strategy_combo.setMinimumHeight(40)
        ...
        self.view_info_btn.setFixedSize(80, 40)
        self.view_info_btn.setCursor(Qt.CursorShape.PointingHandCursor)
        ...
        strategy_layout.addWidget(self.create_styled_label("Strategy:", bold=True))
        strategy_layout.addLayout(strategy_select_layout)
        strategy_layout.addWidget(self.create_styled_label("Description:", bold=True))
        ...
        params_outer_layout.setContentsMargins(5, 15, 5, 5)
        ...
        self.params_layout.addWidget(self.create_styled_label("No strategy selected", color="#888"))
        ...
        self.preset_name_edit.setFixedHeight(35)
        self.preset_name_edit.setStyleSheet(
            """
            QLineEdit {
                background-color: #2d2d2d;
                border: 1px solid #3d3d3d;
        ...
        self.save_preset_btn.setFixedSize(80, 35)
        ...
        self.save_preset_btn.setStyleSheet(self.view_info_btn.styleSheet())

        self.preset_combo = QComboBox()
        self.preset_combo.setFixedHeight(35)
        self.preset_combo.setStyleSheet(self.strategy_combo.styleSheet())

        self.load_preset_btn = QPushButton("📂 Load")
        self.load_preset_btn.setFixedSize(80, 35)
        ...
        self.load_preset_btn.setStyleSheet(self.view_info_btn.styleSheet())
        ...
        asset_layout.setContentsMargins(15, 15, 15, 15)

        self.asset_combo = QComboBox()
        self.asset_combo.setFixedHeight(35)
        self.asset_combo.setStyleSheet(self.strategy_combo.styleSheet())
        ...
        self.timeframe_combo.setFixedHeight(35)
        self.timeframe_combo.setStyleSheet(self.strategy_combo.styleSheet())
        self.timeframe_combo.addItems(["1min", "5min", "15min", "1hour", "4hour", "1day"])
        self.timeframe_combo.setCurrentText("5min")
        self.timeframe_combo.currentTextChanged.connect(self.on_timeframe_changed)

        asset_layout.addWidget(self.create_styled_label("Asset:", bold=True))
        asset_layout.addWidget(self.asset_combo, 1)
        asset_layout.addWidget(self.create_styled_label("Timeframe:", bold=True))
        ...
        saved_layout.setContentsMargins(15, 15, 15, 15)
        ...
        self.saved_strategies_table.setStyleSheet(
            """
            QTableWidget {
                background-color: #252525;
                border: 1px solid #3d3d3d;
    ...
    def create_preview_panel(self):
        ...
        controls_layout.addWidget(self.create_styled_label("Preview length:", bold=True))
            ...
            if hasattr(strategy_class, "get_default_params"):
                ...
                try:
                    ...
                    if hasattr(temp_strategy, "get_params"):
                        ...
                        self.params_layout.addWidget(self.create_styled_label("No configurable parameters for this strategy", color="#888"))
                except Exception as e:
                    self.logger.error(f"Error getting strategy parameters: {e}")
                    self.params_layout.addWidget(self.create_styled_label("Error loading parameters", color="#f48771"))
...
<p style='color: #888; font-size: 11px;'>"""
            ...
            QMessageBox.warning(self, "Error", f"Error al mostrar información de la estrategia:\n{str(e)}")

    def create_param_widgets(self, params):
        """Create parameter input widgets with real-time validation, grouped by prefix"""
        self.param_widgets = {}
        ...
        # Group parameters by prefix (e.g. 'rsi_length' -> 'rsi')
        groups = {}
        misc_params = {}
        ...
        for param_name, param_config in params.items():
            if '_' in param_name:
                ...
                if prefix not in groups:
                    groups[prefix] = {}
                groups[prefix][param_name] = param_config
            else:
                misc_params[param_name] = param_config
                
        # If few groups or few parameters, just show flat
        if len(params) < 6:
            self._add_params_to_layout(params, self.params_layout)
            return

        # Use QToolBox for groups
        toolbox = QToolBox()
        ...
        for group_name, group_params in groups.items():
            ...
            page_layout.setContentsMargins(5, 15, 5, 5)
            self._add_params_to_layout(group_params, page_layout)
            page_layout.addStretch()
            toolbox.addItem(page, f"🔹 {group_name} Parameters")
        ...
        if misc_params:
            ...
            page_layout = QVBoxLayout(page)
            page_layout.setContentsMargins(5, 15, 5, 5)
            self._add_params_to_layout(misc_params, page_layout)
            page_layout.addStretch()
            toolbox.addItem(page, "⚙️ General Parameters")
            
        self.params_layout.addWidget(toolbox)

    def _add_params_to_layout(self, params, layout):
        """Helper to add parameter widgets to a specific layout"""
        for param_name, param_config in params.items():
            ...
            slider.valueChanged.connect(lambda val, name=param_name: self.on_param_changed(name, val * step, "slider"))
            spin_box.valueChanged.connect(lambda val, name=param_name: self.on_param_changed(name, val, "spinbox"))
    ...
    def create_param_widgets_from_strategy(self, params):
        ...
        for param_name, param_info in params.items():
            if isinstance(param_info, dict):
                ...
                converted_params[param_name] = {"min": min_val, "max": max_val, "default": default_val, "step": step}
    ...
    def on_save_preset(self):
        ...
        try:
            params = self.get_current_params()
            preset_data = {"strategy": self.current_strategy, "params": params, "created": datetime.now().isoformat()}
