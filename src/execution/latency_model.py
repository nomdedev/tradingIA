...
class LatencyModel:
    ...
    def calculate_total_latency(
        self, order_type: str = "market", market_volatility: float = 1.0, time_of_day: Optional[int] = None
    ) -> float:
        """
    ...
    def _get_time_of_day_multiplier(self, hour: Optional[int]) -> float:
        ...
        latency_profile = {
            ...
            3: 1.0,  # Night: normal
            4: 1.2,
            5: 1.3,  # Pre-market: building
            6: 1.5,
            7: 1.5,
            8: 1.6,  # Market open: HIGH
            9: 1.3,
            10: 1.2,  # Morning: settling
            11: 1.0,
            12: 1.0,
            13: 1.0,  # Lunch: calm
            14: 1.2,
            15: 1.3,  # Afternoon: picking up
            16: 1.4,
            17: 1.5,  # Market close: HIGH
            18: 1.2,
            19: 1.1,  # After hours: settling
            20: 1.0,
            21: 1.0,
            22: 1.0,
            23: 1.0,  # Evening: normal
        }

        return latency_profile.get(hour, 1.0)

    def apply_latency_to_signal(
        self, signal_time: datetime, data_df, current_index: int, order_type: str = "market", volatility: float = 1.0
    ) -> Dict:
        """
    ...
    def calculate_market_data_staleness(self, data_timestamp: datetime, current_time: datetime) -> float:
        """
    ...
    def get_latency_statistics(self, n_samples: int = 1000) -> Dict:
        ...
        for _ in range(n_samples):
            latency = self.calculate_total_latency(order_type="market", market_volatility=1.0, time_of_day=None)
...
if __name__ == "__main__":
    ...
    for scenario in scenarios:
        latency = model.calculate_total_latency(
            order_type=scenario["order_type"], market_volatility=scenario["vol"], time_of_day=scenario["hour"]
        )
