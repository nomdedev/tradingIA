...
class Order:
    ...
    def add_fill(self, quantity: float, price: float, timestamp: datetime):
        ...
        # Record fill
        self.fills.append({"timestamp": timestamp, "quantity": quantity, "price": price})
...
class OrderManager:
    ...
    def process_orders(
        ...
        for order in list(self.orders.values()):
            ...
            if order.order_type == OrderType.MARKET:
                ...
                filled = self._process_limit_order(
                    order, current_price, current_timestamp, high, low, volume, avg_volume
                )

            elif order.order_type == OrderType.STOP_MARKET:
                filled = self._process_stop_market_order(
                    order, current_price, current_timestamp, high, low, volume, avg_volume
                )

            elif order.order_type == OrderType.TRAILING_STOP:
                filled = self._process_trailing_stop_order(
                    order, current_price, current_timestamp, high, low, volume, avg_volume
                )
    ...
    def _process_market_order(
        self, order: Order, price: float, timestamp: datetime, volume: float, avg_volume: float
    ) -> bool:
        """
    ...
    def _process_limit_order(
        self, order: Order, price: float, timestamp: datetime, high: float, low: float, volume: float, avg_volume: float
    ) -> bool:
        """
    ...
    def _process_stop_market_order(
        ...
        if triggered:
            ...
            if fill_quantity > 0:
                order.add_fill(fill_quantity, fill_price, timestamp)
                self.logger.info(
                    f"Stop market {order.order_id} triggered and filled: {fill_quantity:.4f} @ {fill_price:.2f}"
                )
                return True
...
if __name__ == "__main__":
    ...
    stop_order = manager.create_order(
        symbol="BTC-USD", side=OrderSide.SELL, order_type=OrderType.STOP_MARKET, quantity=0.15, stop_price=48000
    )
