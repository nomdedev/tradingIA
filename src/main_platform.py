...
try:
    from core.api import API_AVAILABLE, app as api_app
except ImportError:
    API_AVAILABLE = False
    api_app = None
...
class TradingPlatform(QMainWindow):
    ...
    def _create_tabs(self):
        ...
        self.tabs.addTab(Tab6LiveMonitor(self, self.backtester), "🔴 Live")
...
• POST /orders - Place orders<br>
            ...
            if old_size.width() > 0 and old_size.height() > 0:  # Skip initial resize
                ...
                if width_diff > 50 or height_diff > 50:
                    if hasattr(self, "session_logger") and self.session_logger:
                        self.session_logger.log_window_event(
                            "resize",
                            {
    ...
    def update_status(self, msg, status_type="info"):
        ...
        status_icons = {"success": "🟢", "warning": "🟡", "error": "🔴", "info": "🔵", "processing": "⏳"}
    ...
    def _load_local_btc_data(self):
        ...
        try:
            ...
            if hasattr(self, "session_logger") and self.session_logger:
                self.session_logger.log_data_loading_event(
                    "auto_load_success",
                    {
                        "symbol": "BTC-USD",
                        "timeframe": timeframe,
        ...
        try:
            ...
            if hasattr(self, "tabs"):
                ...
                if hasattr(tab1, "on_data_loaded"):
                    ...
                    # Log UI update
                    if hasattr(self, "session_logger") and self.session_logger:
                        self.session_logger.log_ui_event(
                            "data_loaded_notification",
                            {"tab": "Tab1_DataManagement", "data_keys": list(self.data_dict.keys())},
                        )
        except Exception as e:
