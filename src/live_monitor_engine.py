...
class LiveMonitorEngine(QObject):
    ...
    def __init__(self, alpaca_api_key, alpaca_secret_key):
        ...
        logging.basicConfig(
            filename=str(LOGS_DIR / "live_monitor.log"), level=logging.INFO, format="%(asctime)s - %(levelname)s - %(message)s"
        )
    ...
    def _initialize_api_with_retry(self, api_key, secret_key, max_retries=3):
        ...
        for attempt in range(max_retries):
            try:
                ...
                self.logger.warning(f"API initialization failed (attempt {attempt + 1}/{max_retries}): {e}")
    ...
    def _rate_limit_check(self):
        ...
        if len(self._api_call_timestamps) >= self._max_calls_per_minute:
            ...
            self._api_call_timestamps = [ts for ts in self._api_call_timestamps if time.time() - ts < 60]
    ...
    def monitor_signals(self, strategy_engine, data_manager, interval_sec=300):
        ...
        self.monitor_thread = threading.Thread(
            target=self._monitor_loop, args=(strategy_engine, data_manager, interval_sec)
        )
    ...
    def _monitor_loop(self, strategy_engine, data_manager, interval_sec):
        ...
        while self.monitoring:
            try:
                # Fetch latest data
                df_latest = data_manager.load_alpaca_data(
                    symbol="BTCUSD", timeframe="5Min", start_date="2024-01-01"
                )  # Last few days
    ...
    def _calculate_live_metrics(self, df):
        ...
        try:
            ...
            sharpe = (
                (excess_returns.mean() / excess_returns.std()) * np.sqrt(252 * 24 * 12)
            ...
            return {"sharpe": sharpe, "win_rate": win_rate, "max_dd": max_dd, "trades_today": 0}  # Placeholder
        except Exception as e:
