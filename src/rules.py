...
def check_long_conditions(
    df_5m: pd.DataFrame, idx: int, min_score: int = 4
...
def should_exit_position(df_5m: pd.DataFrame, idx: int, position: Dict[str, Any]) -> Tuple[bool, str]:
    """
...
def update_trailing_stop(position: Dict[str, Any], current_price: float, atr: float) -> Dict[str, Any]:
    """
...
if __name__ == "__main__":
    ...
    df_test = pd.DataFrame(
        {
            "timestamp": dates,
            "close": 45000 + np.random.standard_normal(500) * 100,
            "high": 45000 + np.random.standard_normal(500) * 100 + 50,
            "low": 45000 + np.random.standard_normal(500) * 100 - 50,
            "volume": 500 + np.random.standard_normal(500) * 50,
            "ATR": 150 + np.random.standard_normal(500) * 10,
            "ema_210_1h": 44500 + np.random.standard_normal(500) * 50,
            "ema_18_5m": 45000 + np.random.standard_normal(500) * 80,
            "ema_48_15m": 44900 + np.random.standard_normal(500) * 70,
            "ifvg_bull": np.random.choice([True, False], 500, p=[0.1, 0.9]),
            "ifvg_bear": np.random.choice([True, False], 500, p=[0.1, 0.9]),
            "vp_val": 44800 + np.random.standard_normal(500) * 50,
            "vp_vah": 45200 + np.random.standard_normal(500) * 50,
            "vp_poc_1h": 45000 + np.random.standard_normal(500) * 30,
            "atr_1h": 300 + np.random.standard_normal(500) * 20,
        }
    )
