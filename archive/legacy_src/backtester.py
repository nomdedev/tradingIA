...
import warnings
from core.council import Council  # Import Council
from core.risk.risk_manager import RiskManager  # Import RiskManager
...
class AdvancedBacktester:
    ...
    def __init__(
        ...
        Args:
            ...
            council: Optional Council instance for decision making
            risk_manager: Optional RiskManager instance for hard limits
            execution_mode: 'close' (immediate) or 'next_open' (realistic)
            dynamic_slippage: Whether to use volatility-based slippage
        """
        self.capital = capital
        self.commission = commission
        self.slippage = slippage
        self.council = council  # Store council instance
        self.risk_manager = risk_manager  # Store risk manager instance
        self.execution_mode = execution_mode
        self.dynamic_slippage = dynamic_slippage

        # Initialize data handler
        # Prefer SQLDataHandler, fall back to MultiTFDataHandler if needed
        try:
            self.data_handler = SQLDataHandler()
        except Exception as e:
            logging.warning(f"Failed to init SQLDataHandler: {e}. Falling back to MultiTFDataHandler.")
    ...
    def run_optimized_backtest(
        self, dfs: Dict[str, pd.DataFrame], params: Dict[str, Any], use_mtf: bool = True
    ) -> Dict[str, Any]:
        """
        ...
        from core.data.indicators import calculate_ifvg_enhanced, volume_profile_advanced, emas_multi_tf
        ...
        df_5m["uptrend_1h"] = (df_5m["ema_trend_1h"] == 1) if "ema_trend_1h" in df_5m.columns else True
        df_5m["momentum_15m"] = (df_5m["ema_trend_15m"] == 1) if "ema_trend_15m" in df_5m.columns else True
        ...
        # Run backtest
        trades, equity_curve = self._execute_trades(df_5m, bull_signals, bear_signals, confidence, params)

        # Calculate metrics
        metrics = self._calculate_metrics(equity_curve, trades, df_5m)

        return {"metrics": metrics, "trades": trades, "equity_curve": equity_curve, "params": params}

    def _calculate_slippage(self, price: float, bar: pd.Series) -> float:
        """Calculate slippage based on volatility if enabled."""
        if not self.dynamic_slippage:
            return price * self.slippage

        # Dynamic slippage: 10% of ATR or 0.1% of price * volatility factor
        if "ATR" in bar:
            # High volatility -> High slippage
            return bar["ATR"] * 0.1
        else:
            # Fallback
            return price * self.slippage * (1 + np.random.random())
    ...
    def _execute_trades(
        ...
        Supports realistic execution (latency, slippage).
        """
        ...
        # Pending order state for 'next_open' execution
        pending_signal = 0  # 0, 1, -1
        ...
        for i in range(len(df)):
            ...
            current_open = df["open"].iloc[i] if "open" in df.columns else current_close
            current_high = df["high"].iloc[i] if "high" in df.columns else current_close
            current_low = df["low"].iloc[i] if "low" in df.columns else current_close
            ...
            if self.risk_manager:
                ...
                self.risk_manager.update_state(equity[-1], current_date)

            # -------------------------------------------------------
            # 1. Execute Pending Orders (at Open of current bar)
            # -------------------------------------------------------
            if self.execution_mode == "next_open" and pending_signal != 0 and position == 0:
                # Execute at Open
                exec_price = current_open
                slip = self._calculate_slippage(exec_price, df.iloc[i])

                # Calculate Position Size (using ATR from previous bar ideally, but current is fine for approx)
                # Risk Amount
                risk_amount = capital * risk_per_trade

                # Stop Loss Distance
                sl_dist = current_atr * sl_multiplier

                if pending_signal == 1:
                    entry_price = exec_price + slip
                ...
                # Cap exposure
                max_pos_value = capital * max_exposure
                if position_size * entry_price > max_pos_value:
                ...
                pending_signal = 0  # Reset

            # -------------------------------------------------------
            # 2. Check Exits (Intra-bar or Close)
            # -------------------------------------------------------
            if position != 0:
                pnl = 0
                ...
                # Check High/Low for SL/TP (Intra-bar simulation)
                # For Long: Low hits SL, High hits TP
                # For Short: High hits SL, Low hits TP

                if position == 1:
                    if current_low <= stop_loss:
                        exit_price = stop_loss  # Assume filled at SL
                        # Apply slippage on stop? Usually yes.
                        slip = self._calculate_slippage(exit_price, df.iloc[i])
                        exit_price -= slip
                        exit_reason = "stop_loss"
                        exit_triggered = True
                    elif current_high >= take_profit:
                        exit_price = take_profit
                        # Limit order usually no slippage or positive slippage
                        exit_reason = "take_profit"
                        exit_triggered = True
                    elif trailing_activated:
                        new_stop = current_close - (trailing_delta_rr * risk_amount / position_size)
                ...
                if exit_triggered:
                    ...
                    entry_cost = entry_price * position_size * self.commission
                    exit_cost = exit_price * position_size * self.commission
            ...
            # -------------------------------------------------------
            # 3. Generate New Signals (at Close)
            # -------------------------------------------------------
            if position == 0 and pending_signal == 0:
                # Prepare context for Council
                current_exposure = position_size * entry_price if position != 0 else 0.0

                context = {
                    "price": current_close,
                    "atr": current_atr,
                    "confidence": current_confidence,
                    "signal_confidence": current_confidence,  # Map for Council rule
                    "capital": capital,
                    "equity": equity[-1],
                    "leverage": 0.0,  # No position open
                    "current_drawdown_pct": (max(equity) - equity[-1]) / max(equity) if equity else 0.0,
                    "uptrend_1h": df.get("uptrend_1h", True).iloc[i],
                    "momentum_15m": df.get("momentum_15m", True).iloc[i],
                }
                ...
                if signal_detected != 0:
                    ...
                    if self.risk_manager:
                        risk_check = self.risk_manager.check_order(
                            {"type": "long" if signal_detected == 1 else "short", "price": current_close}
                        )
                    ...
                    if self.council:
                        ...
                        # Check if approved (custom key) or decision matches/neutral
                        # If 'approved' key exists (legacy/custom), use it
                        if "approved" in decision:
                            if not decision["approved"]:
                                council_allowed = False
                        else:
                            # Standard Council.decide returns 'decision' (-1, 0, 1)
                            # Reject if Vetoed
                            if decision.get("decision") == 0:
                                council_allowed = False
                            # Reject if Council opposes signal or is neutral
                            elif signal_detected == 1 and decision.get("decision") != 1:
                                council_allowed = False
                            elif signal_detected == -1 and decision.get("decision") != -1:
                                council_allowed = False

                    if risk_allowed and council_allowed:
                        if self.execution_mode == "close":
                            ...
                            # Queue for Next Open
                            pending_signal = signal_detected
                            pending_confidence = current_confidence

        return pd.DataFrame(trades), pd.Series(equity)

    def _execute_trades_legacy(
        self,
        df: pd.DataFrame,
        bull_signals: pd.Series,
        bear_signals: pd.Series,
        ...
        for i in range(len(df)):
            ...
            if position == 0:
                ...
                if bull_signals.iloc[i]:
                    ...
                    risk_allowed = True
                    if self.risk_manager:
                        risk_check = self.risk_manager.check_order({"type": "long", "price": current_price})
                        if not risk_check["allowed"]:
                            risk_allowed = False
                            # Log rejection reason if needed

                    # Council Decision Check
                    council_decision = {"decision": 1}  # Default approve if no council
                    if self.council and risk_allowed:
                        context["signal_type"] = "long"
                        council_decision = self.council.decide(context)

                    if risk_allowed and council_decision["decision"] == 1:
                ...
                # Short signal
                    ...
                    risk_allowed = True
                    if self.risk_manager:
                        risk_check = self.risk_manager.check_order({"type": "short", "price": current_price})
                        if not risk_check["allowed"]:
                            risk_allowed = False
            ...
            if position != 0 and not trailing_activated:
                ...
                if current_pnl >= trailing_start_rr * risk_amount:
                    ...
                    if position == 1:
                        trailing_stop = current_price - (trailing_delta_rr * risk_amount / position_size)
                    else:
                        trailing_stop = current_price + (trailing_delta_rr * risk_amount / position_size)
    ...
    def _calculate_metrics(
        self, equity_curve: pd.Series, trades_df: pd.DataFrame, df: pd.DataFrame
    ) -> Dict[str, float]:
        """
    ...
    def walk_forward_optimization(
        self, dfs: Dict[str, pd.DataFrame], n_periods: int = 6, train_split: float = 0.7
    ) -> Dict[str, Any]:
        """
        ...
        print(f"🚀 Starting walk-forward optimization ({n_periods} periods, {train_split:.0%} train)...")
        ...
        for period in range(n_periods):
            ...
            print(f"📊 Period {period + 1}/{n_periods}: {train_df.index[0].date()} to {test_df.index[-1].date()}")
            ...
            print(f"   Trades: {test_result['metrics']['total_trades']}")
        ...
        print(f"✅ Walk-forward complete. Avg Calmar: {np.mean(test_calmars):.3f}, Degradation: {degradation:.3f}")
    ...
    def _bayesian_optimization(self, dfs: Dict[str, pd.DataFrame]) -> Dict[str, Any]:
        ...
        res = gp_minimize(
            objective,
            param_space,
            n_calls=OPTIMIZATION_CONFIG["bayesian"]["n_calls"],
            n_random_starts=OPTIMIZATION_CONFIG["bayesian"]["n_random_starts"],
            random_state=OPTIMIZATION_CONFIG["bayesian"]["random_state"],
            verbose=False,
        )
    ...
    def monte_carlo_simulation(
        self, dfs: Dict[str, pd.DataFrame], params: Dict[str, Any], n_runs: int = 500, noise_pct: float = 0.1
    ) -> Dict[str, Any]:
        """
        ...
        return {"statistics": mc_stats, "all_results": results, "n_runs": n_runs, "noise_pct": noise_pct}

    def _add_noise_to_data(self, dfs: Dict[str, pd.DataFrame], noise_pct: float) -> Dict[str, pd.DataFrame]:
        """
        ...
        for tf, df in dfs.items():
            ...
            df_noisy["high"] = np.maximum(df_noisy[["open", "close", "high"]].max(axis=1), df_noisy["high"])
            df_noisy["low"] = np.minimum(df_noisy[["open", "close", "low"]].min(axis=1), df_noisy["low"])
    ...
    def stress_test_scenarios(
        self, dfs: Dict[str, pd.DataFrame], params: Dict[str, Any], scenarios: List[str] = None
    ) -> Dict[str, Any]:
        """
        ...
        for scenario in scenarios:
            ...
            print(f"   {scenario}: Calmar={metrics['calmar_ratio']:.3f}, DD={metrics['max_drawdown']:.1%}")

        # Overall stress score
        survival_rate = sum(1 for r in stress_results.values() if r["survival"]) / len(stress_results)
        ...
        stress_results["summary"] = {"survival_rate": survival_rate, "avg_max_dd": avg_dd, "stress_score": stress_score}

        print(f"✅ Stress tests complete. Survival: {survival_rate:.1%}, Score: {stress_score:.3f}")

        return stress_results

    def _apply_stress_scenario(self, dfs: Dict[str, pd.DataFrame], scenario: str) -> Dict[str, pd.DataFrame]:
        """
        ...
        for tf, df in dfs.items():
            ...
            if scenario == "high_vol":
                ...
                crash_factor[crash_point : crash_point + crash_duration] = np.linspace(0.8, 0.95, crash_duration)
            ...
            df_stressed["high"] = np.maximum(df_stressed[["open", "close", "high"]].max(axis=1), df_stressed["high"])
            df_stressed["low"] = np.minimum(df_stressed[["open", "close", "low"]].min(axis=1), df_stressed["low"])
    ...
    def run_complete_optimization(
        self, symbol: str = "BTCUSD", start_date: str = "2023-01-01", end_date: str = "2024-12-31"
    ) -> Dict[str, Any]:
        """
...
if __name__ == "__main__":
    ...
    metrics = backtester.run_backtest(
        use_stop_loss=True, use_take_profit=True, sl_atr_multiplier=1.5, tp_risk_reward=2.0
    )
