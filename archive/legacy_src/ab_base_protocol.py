...
class ABTestingBase:
    ...
    def split_data_random(self, df: pd.DataFrame, split_ratio: float = 0.5) -> Tuple[pd.DataFrame, pd.DataFrame]:
        """
        ...
        # Shuffle dates
        shuffled_dates = unique_dates.to_series().sample(frac=1, random_state=self.random_seed).values
        ...
        logger.info(
            f"Data split: A={len(df_a)} bars ({len(dates_a)} days), " f"B={len(df_b)} bars ({len(dates_b)} days)"
        )

        return df_a, df_b

    def run_backtest_vectorbt(self, df: pd.DataFrame, signals: pd.Series, strategy_name: str) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            logger.info(f"{strategy_name} backtest: {total_trades} trades, " f"Sharpe={sharpe:.3f}, Win={win_rate:.1%}")
    ...
    def _run_backtest_simple(self, df: pd.DataFrame, signals: pd.Series, strategy_name: str) -> Dict[str, Any]:
        """
        ...
        if trades:
            ...
            sharpe_ratio = (
                (excess_pnl.mean() / excess_pnl.std()) * np.sqrt(periods_per_year) if excess_pnl.std() > 0 else 0.0
            )
        ...
        logger.info(f"{strategy_name} backtest: {total_trades} trades, Sharpe={sharpe_ratio:.3f}, Win={win_rate:.1%}")
    ...
    def run_aa_test(self, df: pd.DataFrame, signals: pd.Series, n_runs: int = 5) -> Dict[str, Any]:
        ...
        logger.info(f"A/A test: false positive rate={false_positives:.1%}, bias detected={aa_results['bias_detected']}")

        return aa_results

    def make_decision(self, results_a: Dict, results_b: Dict, stats: Dict, min_trades: int = 100) -> Dict[str, Any]:
        """
        ...
        if p_value < 0.05 and superiority > 0.6 and sharpe_diff > 0.2:
            ...
            reason = (
                f"B significantly better (p={p_value:.3f}, superiority={superiority:.1%}, ΔSharpe={sharpe_diff:.3f})"
            )
...
def ab_protocol(
    ...
    results_a = protocol.run_backtest_vectorbt(df_a, variant_a_signals.loc[df_a.index], "Variant A (IFVG)")
    results_b = protocol.run_backtest_vectorbt(df_b, variant_b_signals.loc[df_b.index], "Variant B (RSI/BB)")
    ...
    results = {
        ...
        "variant_a": {"name": "IFVG Base", "results": results_a},
        "variant_b": {"name": "RSI/BB Alternative", "results": results_b},
