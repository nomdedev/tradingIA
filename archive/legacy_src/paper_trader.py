...
from config.config import ALPACA_CONFIG, TRADING_CONFIG, PAPER_TRADING_CONFIG, get_config
...
class PaperTrader:
    ...
    def __init__(
        ...
        self.trading_client = TradingClient(
            api_key=ALPACA_CONFIG["api_key"], secret_key=ALPACA_CONFIG["secret_key"], paper=True  # PAPER TRADING MODE
        )
        ...
        logger.info(f"Check interval: {self.check_interval}s, Risk/trade: {self.risk_per_trade*100}%")
    ...
    def get_current_price(self, symbol: str = None) -> float:
        ...
        try:
            ...
            request = CryptoBarsRequest(symbol_or_symbols=symbol, timeframe=TimeFrame(1, TimeFrameUnit.Minute), limit=1)
    ...
    def place_market_order(self, side: OrderSide, quantity: float) -> Optional[str]:
        ...
        try:
            order_data = MarketOrderRequest(symbol=self.symbol, qty=quantity, side=side, time_in_force=TimeInForce.GTC)

            order = self.trading_client.submit_order(order_data)
            logger.info(f"Market order placed: {side.value} {quantity} {self.symbol}, ID: {order.id}")
    ...
    def place_limit_order(self, side: OrderSide, quantity: float, limit_price: float) -> Optional[str]:
        """Coloca orden limitada"""
        try:
            order_data = LimitOrderRequest(
                symbol=self.symbol, qty=quantity, side=side, time_in_force=TimeInForce.GTC, limit_price=limit_price
            )

            order = self.trading_client.submit_order(order_data)
            logger.info(f"Limit order placed: {side.value} {quantity} @ ${limit_price}, ID: {order.id}")
    ...
    def open_position(self, signal: int, current_price: float, stop_loss: float, take_profit: float, confidence: float):
        """Abre nueva posición"""
    ...
    def close_position(self, symbol: str, reason: str = "signal"):
        ...
        if order_id:
            ...
            logger.info(f"  P&L: ${position.unrealized_pnl:,.2f} ({position.unrealized_pnl_percent*100:.2f}%)")
    ...
    def check_entry_signals(self, df: pd.DataFrame):
        ...
        if self.symbol in self.positions:
            ...
            if (signal > 0 and current_position.side == "short") or (signal < 0 and current_position.side == "long"):
            ...
            while self.is_running:
                ...
                try:
                    df = self.data_fetcher.get_historical_data(
                        symbol=self.symbol, timeframe="5Min", limit=500  # Suficiente para indicadores
                    )
    ...
    def print_status(self):
        ...
        if self.positions:
            ...
            for symbol, pos in self.positions.items():
                ...
                print(f"    P&L: ${pos.unrealized_pnl:,.2f} ({pos.unrealized_pnl_percent*100:+.2f}%)")
...
if __name__ == "__main__":
    ...
    try:
        ...
        trader = PaperTrader(
            symbol="BTCUSD", initial_capital=10000, risk_per_trade=0.01, check_interval=5  # 5 seconds for testing
        )
