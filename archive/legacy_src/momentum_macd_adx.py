...
class MACDADxStrategy(Strategy):
    ...
    def next(self):
        ...
        htf_uptrend = self.ema210_htf[-1] > self.ema210_htf[-2] if len(self.ema210_htf) > 1 else True
        htf_downtrend = self.ema210_htf[-1] < self.ema210_htf[-2] if len(self.ema210_htf) > 1 else False
...
def calculate_metrics(bt):
    ...
    rf_daily = 0.04 / 252  # Risk-free rate diario (4% anual)
    ...
    if len(downside_returns) > 0 and np.std(downside_returns) > 0:
        ...
        sortino = 0.0  # Cambiar de inf a 0 si no hay downside
...
def optimize_strategy(train_data):
    ...
    if OPTIMIZATION_AVAILABLE:
        ...
        best_sharpe = -1000
        best_params = {"adx_thresh": 25.0, "vol_multiplier": 1.2, "sl_atr_mult": 1.5, "tp_rr_mult": 2.2}
            ...
            for vol in vol_options:
                for sl in sl_options:
                    for tp in tp_options:
                        try:
                            params = {"adx_thresh": adx, "vol_multiplier": vol, "sl_atr_mult": sl, "tp_rr_mult": tp}
                            bt = Backtest(train_data, MACDADxStrategy, cash=10000, commission=0.0005)
                            bt.run(**params)
                            metrics = calculate_metrics(bt)
...
def ab_test_vs_rsi_bb(df, rsi_bb_signals):
    ...
    t_stat = (np.mean(macd_returns) - np.mean(rsi_bb_returns)) / np.sqrt(
        np.var(macd_returns) / len(macd_returns) + np.var(rsi_bb_returns) / len(rsi_bb_returns)
    )
