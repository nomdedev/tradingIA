...
class LSTMModel:
    ...
    def build_model(self):
        ...
        model = Sequential(
            [
                LSTM(self.lstm_units, input_shape=(self.lookback, self.n_features), return_sequences=True),
    ...
    def train(self, df_train, target_horizon=5):
        ...
        # Preparar secuencias
        X, y = self.prepare_sequences(pd.DataFrame(scaled_features, index=features.index), target, self.lookback)
        ...
        history = self.model.fit(
            X, y, epochs=100, batch_size=32, validation_split=0.2, callbacks=[early_stopping], verbose=0
        )
    ...
    def predict(self, df_current):
        ...
        X, _ = self.prepare_sequences(
            pd.DataFrame(scaled_features, index=features.index), pd.Series(index=features.index), self.lookback
        )
...
class LSTMMReversionStrategy(Strategy):
    ...
    def next(self):
        ...
        bullish_reversion = (
            current_prediction > self.prediction_threshold and prev_prediction < -self.prediction_threshold
        )
        bearish_reversion = (
            current_prediction < -self.prediction_threshold and prev_prediction > self.prediction_threshold
        )
...
def ab_test_vs_traditional_mr(df, traditional_mr_signals):
    ...
    t_stat = (np.mean(lstm_returns) - np.mean(traditional_returns)) / np.sqrt(
        np.var(lstm_returns) / len(lstm_returns) + np.var(traditional_returns) / len(traditional_returns)
    )
...
def robustness_analysis(walk_forward_results):
    ...
    return {
        ...
        "overall_robust": (
            np.std(sharpes) < 0.2 and np.mean(sortinos) > 1.5 and np.mean(ulcers) < 10 and np.mean(accuracies) > 0.55
        ),
    }
...
def sensitivity_analysis(df, base_params=None, n_tests=5):
    ...
    if base_params is None:
        base_params = {"prediction_threshold": 0.02, "confidence_threshold": 0.7, "max_holding_period": 10}
