...
class StrategyOptimizer:
    ...
    def _calculate_calmar(
        self, equity_curve: pd.Series, periods_per_year: int = 72576  # Correcto: 252 días trading
    ) -> float:
        """
    ...
    def _run_backtest_with_params(self, dfs: Dict[str, pd.DataFrame], params: Dict[str, Any]) -> Dict[str, Any]:
        """
    ...
    def param_sensitivity_heatmap(
        ...
        # Grid search
        results = {metric: pd.DataFrame(index=param1_range, columns=param2_range) for metric in metrics_to_plot}
        ...
        for p1_val in param1_range:
            for p2_val in param2_range:
                ...
                logger.info(f"   Combo {combo_count}/{total_combos}: {param1_name}={p1_val}, {param2_name}={p2_val}")
        ...
        for idx, metric in enumerate(metrics_to_plot):
            sns.heatmap(results[metric], annot=True, fmt=".3f", cmap="RdYlGn", ax=axes[idx], cbar_kws={"label": metric})
        ...
        best_sharpe_idx = results["sharpe_ratio"].stack().idxmax() if "sharpe_ratio" in results else None
        if best_sharpe_idx:
            logger.info(
    ...
    def bayes_opt_sharpe(
        ...
        def objective(param_values):
            ...
            logger.info(f"   Eval: {dict(zip(param_names, param_values))} → {target_metric}={score:.3f}")

            return -score  # Negativo porque gp_minimize minimiza

        # Create search space
        space = [Real(low, high, name=name) for (low, high), name in zip(param_bounds, param_names)]
