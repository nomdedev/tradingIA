    ...
    def next(self):
        ...
        if self.position.size == 0:
            ...
            if zscore < -1.5:  # Less aggressive threshold (was -1.0)
                ...
                if stop_distance > 0:
                    ...
                    size_fraction = min(max(position_value / self.equity, 0.02), 0.30)  # 2%-30% of equity
        ...
        # Exit conditions - only exit if we have a position
            ...
            if self.position_type == "long_spread":
                ...
                if (
                    zscore > 0.8
                    or self.data.Close[idx] < self.entry_price - stop_distance
                    or bars_in_position >= max_holding_bars
                ):
                    print(
...
def walk_forward_pairs_test(btc_df: pd.DataFrame, eth_df: pd.DataFrame, params: Dict = None) -> Dict:
    ...
    if params:
        PairsTradingCointegrationStrategy.spread_zscore_threshold = params.get("zscore_threshold", 2.0)
    ...
    try:
        bt = Backtest(test_data, PairsTradingCointegrationStrategy, cash=10000, commission=0.001)
        result = bt.run()
...
def run_pairs_crypto_analysis(data_path_btc: str = None, data_path_eth: str = None):
    ...
    try:
        ...
        coint_results = {"coint_p_value": 0.0, "adf_p_value": 0.0, "half_life": 1000, "is_cointegrated": True}
    ...
    best_params = {"zscore_threshold": 2.0, "vol_mult": 1.2, "risk_mult": 1.5, "tp_rr": 2.2}
    ...
    print(f"OOS Win Rate: {oos_win_rate if isinstance(oos_win_rate, str) else f'{oos_win_rate:.1%}'}")
    ...
    if len(trades_df) > 0:
        ...
        metrics = {"win_rate": win_rate, "total_return": total_return, "num_trades": num_trades, "avg_trade": avg_trade}
    else:
        metrics = {}
    ...
    # Equity curve
    axes[0, 0].plot(result._equity_curve.Equity if hasattr(result, "_equity_curve") else range(len(df)))
    ...
    axes[1, 0].text(0.5, 0.5, "Monthly Returns\nAnalysis", ha="center", va="center", transform=axes[1, 0].transAxes)
    ...
    """
    axes[1, 1].text(
        0.1, 0.9, metrics_text, transform=axes[1, 1].transAxes, verticalalignment="top", fontfamily="monospace"
    )
