...
# Page configuration
st.set_page_config(
    page_title="BTC IFVG Trading Dashboard", page_icon="📊", layout="wide", initial_sidebar_state="expanded"
)
...
def load_market_data(symbol="BTCUSD", timeframe="5Min", days_back=30):
    ...
    try:
        ...
        df = fetcher.get_historical_data(symbol=symbol, timeframe=timeframe, start_date=start_date, end_date=end_date)
...
def plot_trades_distribution(trades_df):
    ...
    # Histogram
    fig.add_trace(
        go.Histogram(x=trades_df["pnl"], nbinsx=30, name="P&L Distribution", marker_color="#1f77b4", opacity=0.7)
    )

    # Add mean line
    mean_pnl = trades_df["pnl"].mean()
    fig.add_vline(x=mean_pnl, line_dash="dash", line_color="red", annotation_text=f"Mean: ${mean_pnl:.2f}")

    fig.update_layout(
        title="Trade P&L Distribution", xaxis_title="P&L ($)", yaxis_title="Count", template="plotly_white", height=400
    )
...
def plot_candlestick_with_signals(df, max_bars=500):
    ...
    if "EMA20" in df.columns:
        fig.add_trace(
            go.Scatter(x=df.index, y=df["EMA20"], mode="lines", name="EMA20", line=dict(color="blue", width=1)),
            row=1,
            col=1,
        )
    ...
    if "EMA50" in df.columns:
        fig.add_trace(
            go.Scatter(x=df.index, y=df["EMA50"], mode="lines", name="EMA50", line=dict(color="orange", width=1)),
            row=1,
            col=1,
        )
...
def display_metrics(metrics):
    ...
    with col1:
        st.metric("Total Return", f"{metrics.get('total_return', 0):.2f}%", delta=None)
    ...
    with col4:
        ...
        st.metric("Avg Risk/Reward", f"{metrics.get('avg_rr', 0):.2f}")
...
def display_trades_table(trades_df):
    ...
    styled_df = display_df[available_cols].style.applymap(
        color_pnl, subset=["pnl", "pnl_percent"] if "pnl" in display_df.columns else []
    )
...
def main():
    ...
    with st.sidebar:
        ...
        # Mode selection
        mode = st.selectbox("Dashboard Mode", ["Backtest Results", "Paper Trading", "Live Analysis"], index=0)
    ...
    if mode == "Backtest Results":
        ...
        # Charts
        tab1, tab2, tab3, tab4 = st.tabs(["📊 Equity Curve", "📉 Drawdown", "📊 P&L Distribution", "📋 Trades"])

        with tab1:
            initial_capital = metrics.get("final_capital", 10000) if metrics else 10000
            st.plotly_chart(plot_equity_curve(equity_df, initial_capital), use_container_width=True)

        with tab2:
            st.plotly_chart(plot_drawdown(equity_df), use_container_width=True)

        with tab3:
            st.plotly_chart(plot_trades_distribution(trades_df), use_container_width=True)
        ...
        if st.button("Load Data", type="primary"):
            with st.spinner("Loading market data..."):
                ...
                if df is not None:
                    ...
                    # Display chart
                    st.plotly_chart(plot_candlestick_with_signals(df), use_container_width=True)
                    ...
                    if len(signals_df) > 0:
                        st.dataframe(signals_df[["signal", "confidence", "Close", "Volume"]], use_container_width=True)
                    else:
