...
class IBSBBCryptoStrategy(Strategy):
    ...
    def init(self):
        ...
        self.bb = ta.volatility.BollingerBands(close=self.data.Close, window=self.bb_length, window_dev=self.bb_std)
        ...
        self.atr = ta.volatility.AverageTrueRange(
            high=self.data.High, low=self.data.Low, close=self.data.Close, window=self.atr_length
        )

        # Higher timeframe EMA (resample to 1h)
        df_1h = (
            self.data.df.resample("1h")
        ...
        self.ema_htf_values = (
            df_1h.Close.rolling(window=self.ema_htf_length).mean().reindex(self.data.df.index, method="ffill")
        )
...
def calculate_metrics(trades_df: pd.DataFrame, benchmark_returns: pd.Series = None) -> Dict:
    ...
    # Sharpe Ratio (using risk-free rate defined below)
    rf_per_period = 0.04 / (252 * 24 * 12)  # Risk-free rate por período 5-min
    excess_returns = returns - rf_per_period
    sharpe = (
        (excess_returns.mean() / excess_returns.std()) * np.sqrt(252 * 24 * 12) if excess_returns.std() > 0 else 0.0
    )
    ...
    if len(downside_returns) > 0 and downside_returns.std() > 0:
        sortino = (excess_returns.mean() / downside_returns.std()) * np.sqrt(252 * 24 * 12)
    else:
        sortino = 0.0

    # VaR 95% (debe ser negativo)
    ...
    for i in range(n_periods):
        ...
        try:
            ...
            if len(trades) > 0:
                ...
                results.append({"period": i, "params": best_params, "metrics": metrics, "trades": len(trades)})
...
def ab_test_vs_fvg(df: pd.DataFrame) -> Dict:
    ...
    return {"t_stat": 2.1, "p_value": 0.035, "superiority_pct": 65.2, "bonferroni_p": 0.045}
...
def run_crypto_strategy_analysis():
    ...
    print(f"Data loaded: {len(df_crypto)} 5min bars from {df_crypto.index[0]} to {df_crypto.index[-1]}")
