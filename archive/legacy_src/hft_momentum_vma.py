...
class HFTMomentumVMAStrategy(Strategy):
    ...
    def init(self):
        ...
        self.kalman = KalmanFilter(
            process_noise=self.kalman_process_noise, measurement_noise=self.kalman_measurement_noise
        )
        ...
        self.volatility = self.I(talib.ATR, self.data.High, self.data.Low, self.data.Close, timeperiod=10)
...
def calculate_metrics(bt):
    ...
    if max_dd < 0:
        calmar = -np.mean(daily_returns) / abs(max_dd) * np.sqrt(252) if len(daily_returns) > 0 else 0
    else:
        calmar = 0
    ...
    holding_periods = (trades["ExitTime"] - trades["EntryTime"]).dt.total_seconds() / 60  # en minutos
    ...
    # Latency impact
    latency_signals = len(strategy_instance.latency_delays) if hasattr(strategy_instance, "latency_delays") else 0
...
def walk_forward_test(df, n_periods=6, train_hours=24, test_hours=6):
    ...
    for i in range(n_periods):
        ...
        bt = Backtest(test_data, HFTMomentumVMAStrategy, cash=10000, commission=0.0002)  # Lower commission for HFT
...
def optimize_strategy(train_data):
    ...
    print(f"Mejores parámetros encontrados: momentum_threshold={result.x:.4f}, Sharpe: {-result.fun:.2f}")
    return best_params
...
def ab_test_vs_simple_momentum(df, simple_momentum_signals):
    ...
    t_stat = (np.mean(hft_returns) - np.mean(simple_returns)) / np.sqrt(
        np.var(hft_returns) / len(hft_returns) + np.var(simple_returns) / len(simple_returns)
    )
...
def robustness_analysis(walk_forward_results):
    ...
    return {
        ...
        "overall_robust": (
            np.std(sharpes) < 0.2 and np.mean(sortinos) > 1.5 and np.mean(ulcers) < 10 and np.mean(latencies) < 0.3
        ),
    }
...
def generate_report(walk_forward_results, ab_results, robustness, sensitivity, anti_overfit):
    ...
    summary = {
        ...
        "recommendation": (
            "Deploy"
            if (
                np.mean(oos_sharpes) > 1.5
                and ab_results["significant"]
                and robustness["overall_robust"]
                and anti_overfit["overfit_risk"] == "low"
            )
