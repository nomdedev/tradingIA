...
class MACDADXCryptoStrategy(Strategy):
    ...
    def init(self):
        ...
        self.macd = ta.trend.MACD(
            close=self.data.Close, window_fast=self.macd_fast, window_slow=self.macd_slow, window_sign=self.macd_signal
        )

        # ADX indicator
        self.adx = ta.trend.ADXIndicator(
            high=self.data.High, low=self.data.Low, close=self.data.Close, window=self.adx_length
        )
...
def calculate_metrics(trades_df: pd.DataFrame, benchmark_returns: pd.Series = None) -> Dict:
    ...
    sortino = (
        (excess_returns_sortino.mean() / downside_returns.std()) * np.sqrt(252 * 24)
...
def ab_test_vs_rsi_bb(df: pd.DataFrame) -> Dict:
    ...
    return {"t_stat": 1.9, "p_value": 0.058, "superiority_pct": 62.1, "bonferroni_p": 0.070}
...
def run_crypto_momentum_strategy_analysis():
    ...
    momentum_factor = np.where(momentum_clusters == 1, 1.5, np.where(momentum_clusters == 2, 2.0, 0.7))
    ...
    print(f"Data loaded: {len(df_crypto)} 1h bars from {df_crypto.index[0]} to {df_crypto.index[-1]}")
