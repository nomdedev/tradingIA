...
class StrategyComparator:
    ...
    def _get_placeholder_strategy(self, strategy_name: str, asset_class: str):
        ...
        strategy_adjustments = {
            "mean_reversion": {"sharpe": 1.0, "win_rate": 0.65, "max_dd": -0.20, "total_return": 0.35},
            "momentum": {"sharpe": 1.2, "win_rate": 0.55, "max_dd": -0.25, "total_return": 0.40},
            "pairs_trading": {"sharpe": 1.5, "win_rate": 0.68, "max_dd": -0.15, "total_return": 0.42},
            "hft": {"sharpe": 1.6, "win_rate": 0.62, "max_dd": -0.12, "total_return": 0.48},
            "lstm": {"sharpe": 1.7, "win_rate": 0.70, "max_dd": -0.18, "total_return": 0.52},
        }
            ...
            for strategy in self.strategy_types:
                try:
                    ...
                    if result:
                        ...
                        print(f"  ✓ {strategy}: Sharpe {result['sharpe']:.2f}, Win {result['win_rate']:.1%}")
                    else:
    ...
    def statistical_significance_test(self, results_df: pd.DataFrame) -> Dict:
        ...
        for strategy in results_df["Strategy"].unique():
            strategy_groups[strategy] = results_df[results_df["Strategy"] == strategy]["Sharpe"].values
        ...
        for i, strat1 in enumerate(strategies):
            for strat2 in strategies[i + 1 :]:
                if len(strategy_groups[strat1]) > 1 and len(strategy_groups[strat2]) > 1:
                    t_stat, p_value = stats.ttest_ind(strategy_groups[strat1], strategy_groups[strat2], equal_var=False)
    ...
    def create_visualizations(self, results_df: pd.DataFrame):
        ...
        sharpe_pivot = results_df.pivot_table(values="Sharpe", index="Strategy", columns="Asset", aggfunc="mean")
        ...
        win_pivot = results_df.pivot_table(values="Win Rate", index="Strategy", columns="Asset", aggfunc="mean")
        ...
        dd_pivot = results_df.pivot_table(values="Max DD", index="Strategy", columns="Asset", aggfunc="mean")
        ...
        metrics_corr = results_df[["Sharpe", "Win Rate", "Max DD", "Total Return", "Calmar", "Volatility"]].corr()
    ...
    def generate_ensemble_recommendations(self, results_df: pd.DataFrame) -> Dict:
        ...
        for asset in self.assets:
            ...
            if len(asset_data) > 0:
                ...
                asset_data["risk_adj"] = asset_data["Sharpe"] / (abs(asset_data["Max DD"]) * asset_data["Volatility"])
        ...
        recommendations["ensemble"] = sorted(ensemble_suggestions, key=lambda x: x["avg_sharpe"], reverse=True)
