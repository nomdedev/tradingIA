...
class RegimeDetectorAdvanced:
    ...
    def _save_config(self):
        ...
        config = {"n_regimes": self.n_regimes, "regime_params": self.regime_params, "regime_names": self.regime_names}
    ...
    def prepare_features_for_hmm(self, df: pd.DataFrame) -> np.ndarray:
        ...
        features["momentum"] = (df["Close"].rolling(10).mean() - df["Close"].rolling(50).mean()) / df["Close"].rolling(
            50
        ).mean()
    ...
    def walk_forward_regime_detection(self, df: pd.DataFrame, train_window: int = 252, test_window: int = 21) -> Dict:
        """
        ...
        for i in range(max(1, n_splits - 10)):  # Últimos 10 splits
            ...
            if garch_success:
                ...
                vol_actual = test_data["Close"].pct_change().rolling(20).std().iloc[-len(vol_forecast) :]
        ...
        for regime_num, regime_name in self.regime_names.items():
            ...
            if len(regime_data) > 0:
                ...
                stats[regime_name] = {
                    ...
                    "sharpe": sharpe,
                    "max_dd": (regime_data["Close"] / regime_data["Close"].expanding().max() - 1).min(),
...
def integrate_regime_detection(df: pd.DataFrame, train_hmm: bool = True) -> pd.DataFrame:
    """
