...
class EnsembleModel:
    ...
    def train(self, df_train, target_horizon=3):
        ...
        # Prepare sequences for LSTM
        X_lstm, y_lstm = self.prepare_sequences(pd.DataFrame(scaled_features, index=features.index), target)
        ...
        self.lstm_model.fit(
            X_lstm, y_lstm, epochs=50, batch_size=32, validation_split=0.2, callbacks=[early_stop], verbose=0
        )

        # Train traditional model
        self.traditional_model.fit(
            X_trad, y_lstm, epochs=50, batch_size=32, validation_split=0.2, callbacks=[early_stop], verbose=0
        )
    ...
    def predict(self, df_current):
        ...
        # LSTM predictions
        X_lstm, _ = self.prepare_sequences(
            pd.DataFrame(scaled_features, index=features.index), pd.Series(index=features.index)
        )
        if len(X_lstm) > 0:
...
class BTCFinalStrategy(Strategy):
    ...
    def init(self):
        ...
        self.kalman = KalmanFilter(process_noise=self.kalman_process_noise, measurement_noise=1e-3)
        ...
        self.bb_upper, self.bb_middle, self.bb_lower = self.I(self._bbands_indicator, self.data.Close)
    ...
    def next(self):
        ...
        # Ensemble signal
        bullish_signal = (
            current_prediction > self.prediction_threshold
            and rsi_oversold
            and bb_lower_touch
            and volume_confirm
            and macd_bullish
        ...
        if bullish_signal and confidence and not self.position.is_long and not self.position.is_short:
        ...
        # Entry short
        elif bearish_signal and confidence and not self.position.is_long and not self.position.is_short:
...
def calculate_advanced_metrics(bt_result):
    ...
    sharpe = (
        (excess_daily_returns.mean() / excess_daily_returns.std()) * np.sqrt(252)
        if len(daily_returns) > 0 and excess_daily_returns.std() > 0
        else 0.0
    )
    ...
    losing_trades = returns[returns < 0]
    profit_factor = (
        winning_trades.sum() / abs(losing_trades.sum()) if len(losing_trades) > 0 and losing_trades.sum() != 0 else 0.0
    )
    ...
    avg_slippage = (
        np.mean(strategy_instance.slippage_costs)
        if hasattr(strategy_instance, "slippage_costs") and strategy_instance.slippage_costs
        else 0
    )
...
def generate_final_report(wf_results):
    ...
    summary = {
        ...
        "final_recommendation": (
            "Deploy" if np.mean(all_sharpes) > 1.5 and np.std(all_sharpes) < 0.3 else "Further Testing"
        ),
    }
...
def generate_final_plots(wf_results, summary):
    ...
    axes[0, 0].set_title("Sharpe Ratio OOS por Periodo (Final Strategy)", fontsize=12, fontweight="bold")
...
def run_final_backtest(df_btc):
    ...
    return {"walk_forward_results": wf_results, "summary": summary, "recommendation": summary["final_recommendation"]}
