                ...
                for holding in holding_options:
                    for risk in risk_options:
                        try:
                            ...
                            bt = Backtest(train_data, PairsTradingStrategy, cash=10000, commission=0.0005)
                            bt.run(**params)
                            metrics = calculate_metrics(bt)
...
def ab_test_vs_mean_reversion(df, mr_signals):
    ...
    t_stat = (np.mean(pairs_returns) - np.mean(mr_returns)) / np.sqrt(
        np.var(pairs_returns) / len(pairs_returns) + np.var(mr_returns) / len(mr_returns)
    )
...
def generate_report(walk_forward_results, ab_results, robustness, sensitivity, anti_overfit, cointegrated_pairs):
...
def run_complete_analysis(df_btc, asset_data_dict=None, mr_signals=None):
    ...
    summary = generate_report(wf_results, ab_results, robustness, sensitivity, anti_overfit, cointegrated_pairs)
