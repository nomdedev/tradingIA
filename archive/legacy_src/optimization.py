...
class ParameterOptimizer:
    ...
    def _load_data(self, start_date, end_date):
        ...
        if self.data is not None:
            ...
            return self.data[(self.data.index >= start_date) & (self.data.index <= end_date)].copy()
        ...
        if max_workers > 1:
            ...
            with ProcessPoolExecutor(max_workers=max_workers) as executor:
                ...
                for combo in combinations:
                    ...
                    future = executor.submit(self._test_params, data.copy(), params, initial_capital)
                ...
                for future, params in futures:
                    try:
                        ...
                        completed += 1
                        if completed % 10 == 0:
                            logger.info(f"Progress: {completed}/{total_tests} ({completed/total_tests*100:.1f}%)")
    ...
    def _test_params(self, data: pd.DataFrame, params: Dict, initial_capital: float) -> Optional[Dict]:
        """
        ...
        try:
            ...
            metrics = backtester.run_backtest(
                use_stop_loss=True, use_take_profit=True, sl_atr_multiplier=sl_atr_mult, tp_risk_reward=tp_rr
            )
    ...
    def walk_forward_analysis(
        ...
        while current_start + timedelta(days=train_period_days + test_period_days) <= end_dt:
            ...
            windows.append(
                {"train_start": current_start, "train_end": train_end, "test_start": train_end, "test_end": test_end}
            )
        ...
        for i, window in enumerate(windows, 1):
            ...
            optimized_params = {k: best_train_params[k] for k in param_keys if k in best_train_params}
            ...
            test_data = self._load_data(
                window["test_start"].strftime("%Y-%m-%d"), window["test_end"].strftime("%Y-%m-%d")
            )
        ...
        if len(window_results) > 0:
            ...
            logger.info(f"Range: [{summary['min_test_metric']:.4f}, {summary['max_test_metric']:.4f}]")
            logger.info("=" * 80)
    ...
    def monte_carlo_simulation(
        self, trades_df: pd.DataFrame, n_simulations: int = 1000, initial_capital: float = 10000.0
    ) -> Dict[str, Any]:
        """
