...
class IBSBBStrategy(Strategy):
    ...
    def init(self):
        ...
        self.ibs = self.I(self._calculate_ibs, self.data.Open, self.data.High, self.data.Low, self.data.Close)
    ...
    def next(self):
        ...
        # Trailing Stop Management
            ...
            if self.position.pl > (self.trail_trigger_rr * abs(self.position.sl - self.position.entry_price)):
...
def ab_test_vs_ifvg(df, ifvg_signals):
    ...
    t_stat = (np.mean(ibs_returns) - np.mean(ifvg_returns)) / np.sqrt(
        np.var(ibs_returns) / len(ibs_returns) + np.var(ifvg_returns) / len(ifvg_returns)
    )
...
def generate_report(walk_forward_results, ab_results, snooping_analysis):
    ...
    summary = {
        ...
        "recommendation": (
            "Deploy"
            if (np.mean(oos_sharpes) > 1.5 and ab_results["significant"] and snooping_analysis["bias_risk"] == "low")
