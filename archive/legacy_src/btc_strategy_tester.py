...
class StrategyComparator:
        ...
        for strategy_key, strategy_info in STRATEGIES.items():
            try:
                with open(strategy_info["file"], "r") as f:
                    data = json.load(f)
                    self.strategies_data[strategy_key] = {"info": strategy_info, "metrics": data}
    ...
    def create_ensemble_recommendations(self):
        ...
        if self.correlation_matrix is not None:
            ...
            available_strategies = [s for s in good_strategies["strategy"] if s in self.correlation_matrix.columns]

            if len(available_strategies) > 1:
                corr_subset = self.correlation_matrix.loc[available_strategies, available_strategies]
                ...
                for i in range(len(available_strategies)):
                    for j in range(i + 1, len(available_strategies)):
                        ...
                        if abs(corr) < 0.3:  # Baja correlación
                            low_corr_pairs.append((available_strategies[i], available_strategies[j], corr))
    ...
    def generate_comparison_report(self):
        ...
        for idx, row in self.ranking_df.iterrows():
            ...
            status = (
                "✅"
                if row["recommendation"] == "Deploy"
                else "⚠️" if row["recommendation"] == "Further Testing" else "❌"
            )
        ...
        if "error" not in stat_results:
            print(f"Top 2 estrategias: {stat_results['top_strategy_1']} vs {stat_results['top_strategy_2']}")
            print(f"t-statistic: {stat_results['t_statistic']:.3f}")
            print(f"p-value: {stat_results['p_value']:.3f}")
            print(f"Significancia: {'✅ p < 0.05' if stat_results['significant'] else '❌ p >= 0.05'}")
            print(f"Tamaño del efecto: {stat_results['effect_size']} (Cohen's d = {stat_results['cohen_d']:.2f})")
        else:
    ...
    def generate_visualizations(self):
        ...
        if self.correlation_matrix is not None:
            sns.heatmap(self.correlation_matrix, annot=True, cmap="coolwarm", center=0, ax=axes[1, 1], square=True)
            axes[1, 1].set_title("Correlation Matrix", fontsize=12, fontweight="bold")
        else:
            axes[1, 1].text(
                0.5, 0.5, "Correlation Matrix\nNot Available", ha="center", va="center", transform=axes[1, 1].transAxes
            )
...
def run_strategy_comparison():
    ...
    results = {
        ...
        "correlation_matrix": (
            comparator.correlation_matrix.to_dict() if comparator.correlation_matrix is not None else None
        ),
        "statistical_test": stat_results,
        "ensemble_recommendations": ensemble_rec,
        "winner": comparator.ranking_df.iloc[0]["name"] if len(comparator.ranking_df) > 0 else None,
    }
