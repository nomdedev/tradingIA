...
class Tab6LiveMonitor(QWidget):
    ...
    def __init__(self, parent_platform, backtester_core):
        ...
        # Initialize Latency Model (Institutional Grade)
        try:
            self.latency_model = LatencyProfile.get_profile("retail_average")
            self.logger.info("Latency model initialized: retail_average")
        except Exception as e:
            self.logger.error(f"Failed to initialize latency model: {e}")
    ...
    def update_simulation(self):
        ...
        # Update Latency (Real Model)
        if self.latency_model:
            latency = int(self.latency_model.calculate_total_latency(market_volatility=1.2))
        else:
