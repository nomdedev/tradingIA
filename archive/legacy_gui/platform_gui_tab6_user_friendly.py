...
class StrategyInfoPanel(QGroupBox):
    ...
    def init_ui(self):
        ...
        self.strategy_desc.setStyleSheet(
            "color: #ccc; font-size: 11px; padding: 8px; background: #1e1e1e; border-radius: 4px;"
        )
...
class DecisionLogPanel(QGroupBox):
    ...
    def __init__(self):
        ...
        self.setToolTip(
            "Este log muestra en tiempo real cada decisión que toma el bot,\nla razón detrás de ella, y los valores de indicadores usados."
        )
        self.init_ui()
...
class StrategySelector(QGroupBox):
    ...
    def init_ui(self):
        ...
        self.strategy_combo.addItems(
            ["RSI Mean Reversion", "MACD Momentum", "Bollinger Bands Breakout", "MA Crossover", "Volume Breakout"]
        )
...
class Tab6LiveMonitoringUserFriendly(QWidget):
    ...
    def init_ui(self):
        ...
        self.status_indicator.setStyleSheet(
            "color: #f48771; font-size: 14px; font-weight: bold; padding: 8px 16px; background-color: #2d2d2d; border-radius: 4px;"
        )
        ...
        self.ticker_combo = QComboBox()
        self.ticker_combo.addItems(
            ["BTC/USD - Bitcoin", "ETH/USD - Ethereum", "AAPL - Apple", "SPY - S&P 500", "QQQ - Nasdaq 100"]
        )
        self.ticker_combo.setStyleSheet(
            """
            QComboBox {
        ...
        self.strategy_info_label.setStyleSheet(
            "color: #ccc; font-size: 12px; padding: 8px; background: #1e1e1e; border-radius: 4px;"
        )
        ...
        self.sharpe_card = MetricCard(
            "Sharpe Ratio", "--", color="#569cd6", tooltip="Relación riesgo/retorno (>1.5 es bueno)"
        )
    ...
    def create_left_panel(self):
        ...
        self.strategy_info.update_strategy(
            self.current_strategy["name"], self.current_strategy["description"], self.current_strategy["parameters"]
        )
    ...
    def create_center_panel(self):
        ...
        self.sharpe_card = MetricCard(
            "Sharpe Ratio", "--", "", "#569cd6", tooltip="Relación riesgo/retorno\n>1.5 = Bueno\n>2.0 = Excelente"
        )
        self.dd_card = MetricCard(
            "Max Drawdown", "--", "%", "#f48771", tooltip="Peor caída desde el pico\nEntre -5% y -10% es aceptable"
        )
        self.win_rate_card = MetricCard(
            "Win Rate", "--", "%", "#4ec9b0", tooltip="% de trades ganadores\n>55% es bueno"
        )
        self.exposure_card = MetricCard("Exposición", "--", "%", "#dcdcaa", tooltip="% de capital en uso")
    ...
    def start_trading(self):
        ...
        # Use selected ticker
        self.monitor_thread = EnhancedLiveMonitorThread(self.current_strategy["name"], self.current_ticker)
    ...
    def stop_trading(self):
        ...
        self.decision_log.add_decision(datetime.now(), "STOPPED", "Trading detenido por el usuario", {})
    ...
    def log_decision(self, decision):
        """Log decision"""
        self.decision_log.add_decision(
            decision["timestamp"], decision["action"], decision["reason"], decision["indicators"]
        )
