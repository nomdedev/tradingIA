...
class SimpleBacktester:
    ...
    def _precalculate_indicators(self, df):
        ...
        self.indicators_cache = {"df_with_indicators": df.copy(), "total_bars": len(df)}
    ...
    def run_backtest(self, df):
        ...
        for idx in range(0, total_bars, step_size):
            ...
            if idx % (1000 * step_size) == 0 or (current_time - last_update_time) > 10 or idx >= total_bars - step_size:
                ...
                print(
                    f"   ⏱️  Velocidad: {bars_per_second:.1f} barras/seg | Tiempo restante: {estimated_remaining/60:.1f} min"
                )
                print()
            ...
            try:
                ...
                vp_data = self.vp_cache.get(
                    idx, {"poc": df.iloc[idx]["close"], "vah": df.iloc[idx]["high"], "val": df.iloc[idx]["low"]}
                )
...
def run_backtest():
    ...
    backtester = SimpleBacktester(strategy=strategy, initial_capital=10000, commission=0.001)  # 0.1%
