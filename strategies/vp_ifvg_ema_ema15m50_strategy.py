"""
Volume Profile + IFVG + EMAs + EMA15m50 Filter Strategy
Enhanced version with 15-minute EMA50 proximity filter

This strategy adds to the original VP+IFVG+EMA:
...
class VPIFVGEmaEMA15m50Strategy(VPIFVGEmaStrategy):
    """
    Enhanced VP+IFVG+EMA strategy with 15-minute EMA50 proximity filter
    
    Additional filtering:
    - Calculates EMA50 on 15-minute timeframe
    - Only allows entries when price within 4% of 15m EMA50
    - Maintains all original VP+IFVG+EMA logic
    """

    def __init__(self, name: str = "VP_IFVG_EMA_EMA15m50_Strategy"):
        super().__init__(name)
    ...
    def set_parameters(self, **params):
        ...
        # Handle EMA15m50 specific parameters locally
        ema_params = {}
        parent_params = {}
        
        for key, value in params.items():
            if key.startswith('ema15m50_') or key == 'use_ema15m50_filter':
                ema_params[key] = value
            else:
                parent_params[key] = value
        
        # Set parent parameters first
        if parent_params:
            super().set_parameters(**parent_params)
        
        # Set EMA15m50 specific parameters
        if 'use_ema15m50_filter' in ema_params:
            self.use_ema15m50_filter = ema_params['use_ema15m50_filter']
        if 'ema15m50_proximity_pct' in ema_params:
            self.ema15m50_proximity_pct = ema_params['ema15m50_proximity_pct']
        if 'ema15m50_period' in ema_params:
            self.ema15m50_period = ema_params['ema15m50_period']
        if 'ema15m50_timeframe' in ema_params:
            self.ema15m50_timeframe = ema_params['ema15m50_timeframe']
