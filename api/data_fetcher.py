...
ÁREA 7: Integrated with DataValidator for mandatory data quality checks
"""
...
try:
    ...
    logging.warning(f"DataValidator not available: {e}")
...
class DataFetcher:
    ...
    def __init__(self, strict_validation: bool = True):
        ...
        Args:
            strict_validation: If True, validation errors will raise exceptions
        """
        ...
        if DATA_VALIDATOR_AVAILABLE:
            ...
            logger.info("✅ DataFetcher initialized with DataValidator (ÁREA 7)")
        else:
            self.validator = None
            logger.warning("⚠️ DataFetcher initialized WITHOUT DataValidator")
    ...
    def get_historical_data(self, symbol='BTCUSD', timeframe='5Min',
        ...
        for attempt in range(self.max_retries):
            try:
                ...
                # ÁREA 7: Validate and clean data with comprehensive checks
                df = self._validate_data(df, timeframe=timeframe)
    ...
    def _validate_data(self, df, timeframe='5Min'):
        """
        Validate and clean OHLCV data using DataValidator (ÁREA 7)
        ...
        Raises:
            ValueError: If strict_validation=True and critical errors found
        """
        if df.empty:
            return df
        ...
        # ÁREA 7: Run comprehensive DataValidator checks
        if self.validator is not None:
            # Prepare DataFrame for validator (needs lowercase columns + timestamp)
            df_for_validation = df.copy()
            df_for_validation.columns = df_for_validation.columns.str.lower()
            df_for_validation['timestamp'] = df_for_validation.index
            
            # Map timeframe to expected frequency
            freq_map = {'1Min': '1min', '5Min': '5min', '15Min': '15min', '1H': '1h', '1Hour': '1h'}
            ...
            # Get summary
            self.last_validation_summary = self.validator.get_summary()
            ...
            if critical_errors:
                ...
                logger.error(f"🚨 CRITICAL validation errors: {error_msgs}")
                if self.strict_validation:
                    raise ValueError(f"Data validation failed (CRITICAL): {error_msgs}")
            
            if errors and self.strict_validation:
                error_msgs = [r.message for r in errors]
                logger.error(f"❌ Validation errors: {error_msgs}")
                # In strict mode, errors also fail
                raise ValueError(f"Data validation failed (ERRORS): {error_msgs}")
    ...
    def _log_validation_results(self, results):
        ...
        for r in results:
            if r.severity == ValidationSeverity.CRITICAL:
                logger.critical(f"🚨 [{r.check_name}] {r.message}")
            elif r.severity == ValidationSeverity.ERROR:
                logger.error(f"❌ [{r.check_name}] {r.message}")
            elif r.severity == ValidationSeverity.WARNING:
                logger.warning(f"⚠️ [{r.check_name}] {r.message}")
            else:
                logger.debug(f"✅ [{r.check_name}] {r.message}")
    
    def get_validation_summary(self) -> dict:
        """Get the last validation summary"""
        return self.last_validation_summary or {"status": "no_validation_run"}
