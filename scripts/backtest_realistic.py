...
class RealisticBacktester:
    ...
    def run_realistic_backtest(self, df_5m, df_15m=None, df_1h=None):
        ...
        for idx in range(len(df_full)):
            ...
            try:
                # Get VP data
                vp_data = vp_cache.get(
                    idx,
                    {
                        "poc": df_full.iloc[idx]["close"],
                        "vah": df_full.iloc[idx]["high"],
                        "val": df_full.iloc[idx]["low"],
                    },
                )
                ...
                if (
                    idx < len(signals["entries"])
                    and signals["entries"].iloc[idx] == 1
                    and self._validate_entry_signal(df_full.iloc[idx], vp_data, signals["signals"].iloc[idx])
                ):
    ...
    def _calculate_realistic_metrics(self):
        ...
        if len(returns) > 0 and returns.std() > 0:
            ...
            sharpe_ratio = (
                (excess_returns.mean() / excess_returns.std()) * np.sqrt(252) if excess_returns.std() > 0 else 0.0
            )
        else:
            sharpe_ratio = 0
...
def load_full_dataset():
    ...
    data_files = {"5m": "data/btc_5Min.csv", "15m": "data/btc_15Min.csv", "1h": "data/btc_1H.csv"}
...
def run_realistic_backtest():
    ...
    for metric_key, metric_name, suffix in metrics_to_show:
        ...
        if metric_key in [
            ...
            print(f"{metric_name:<25} {basic_val:>15.2f} {full_val:>15.2f} {diff:>15.2f} {symbol}")
        elif metric_key in ["profit_factor", "expectancy"]:
            print(f"{metric_name:<25} {basic_val:>15.2f} {full_val:>15.2f} {diff:>15.2f} {symbol}")
        else:
            print(f"{metric_name:<25} {basic_val:>15} {full_val:>15} {diff:>15} {symbol}")
    ...
    if mejora > 5:
        ...
        print("   ✅ Los filtros avanzados aportan valor significativo")
        print("   ✅ POC mejora la calidad sin eliminar oportunidades")
    elif mejora > 0:
        print(f"   ⚠️ Mejora marginal: +{mejora:.2f}%")
        print("   ⚠️ Mejora marginal - considerar simplificar")
    else:
        print(f"   ❌ Empeoramiento: {mejora:.2f}%")
        print("   ❌ Los filtros avanzados no aportan valor")
    ...
    print("=" * 80)
    print(f"   Drawdown Básico: {dd_basic:.2f}%")
    print(f"   Drawdown Completo: {dd_full:.2f}%")
    if dd_full < dd_basic:
    ...
    print("\n📍 ANÁLISIS POC DETALLADO")
    print("=" * 80)
    print(f"   Trades en POC: {poc_trades}")
    print(f"   % Trades en POC: {poc_ratio:.1f}%")
    print(f"   Win Rate en POC: {results['full']['poc_win_rate']:.1f}%")
    ...
    with open("results/realistic_backtest_analysis.md", "w", encoding="utf-8") as f:
        ...
        f.write("|---|---|---|---|---|---|---|\n")
        f.write(
            f"| **Básica** | {results['basic']['total_return_pct']:.2f}% | {results['basic']['total_trades']} | {results['basic']['win_rate']:.1f}% | {results['basic']['profit_factor']:.2f} | {results['basic']['max_drawdown']:.2f}% | - |\n"
        )
        f.write(
            f"| **Completa** | {results['full']['total_return_pct']:.2f}% | {results['full']['total_trades']} | {results['full']['win_rate']:.1f}% | {results['full']['profit_factor']:.2f} | {results['full']['max_drawdown']:.2f}% | {results['full']['poc_trades']} |\n"
        )
        f.write(
            f"| **Diferencia** | {mejora:.2f}% | {results['full']['total_trades'] - results['basic']['total_trades']} | {(results['full']['win_rate'] - results['basic']['win_rate']):.1f}% | {(results['full']['profit_factor'] - results['basic']['profit_factor']):.2f} | {(results['full']['max_drawdown'] - results['basic']['max_drawdown']):.2f}% | - |\n\n"
        )
