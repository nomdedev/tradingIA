...
def run_single_backtest(strategy_class, strategy_name: str, df: pd.DataFrame, initial_capital: float = 10000) -> dict:
    """
...
def main():
    ...
    squeeze_original = run_single_backtest(SqueezeMomentumADXTTMStrategy, "Squeeze+ADX+TTM (Original)", df.copy())
    all_results["squeeze_original"] = squeeze_original

    squeeze_enhanced = run_single_backtest(SqueezeMomentumADXTTMEMA15m50Strategy, "Squeeze+ADX+TTM+EMA15m50", df.copy())
    ...
    vp_original = run_single_backtest(VPIFVGEmaStrategy, "VP+IFVG+EMA (Original)", df.copy())
    all_results["vp_original"] = vp_original

    vp_enhanced = run_single_backtest(VPIFVGEmaEMA15m50Strategy, "VP+IFVG+EMA+EMA15m50", df.copy())
    ...
    print(f"\n📊 Resumen General:")
    print(
        f"   • Squeeze+ADX+TTM: {squeeze_comparison['summary']['improved_metrics']}/{squeeze_comparison['summary']['total_metrics']} métricas mejoradas"
    )
    print(
        f"   • VP+IFVG+EMA: {vp_comparison['summary']['improved_metrics']}/{vp_comparison['summary']['total_metrics']} métricas mejoradas"
    )
    print(f"\n💾 Archivos generados:")
    print(f"   • {json_file}")
    print(f"   • {csv_file}")
