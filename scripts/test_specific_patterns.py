...
import pandas as pd
from scripts.conditional_pattern_evaluator import ConditionalPatternEvaluator, Pattern, Condition, ConditionType
...
def test_specific_patterns():
    ...
    evaluator = ConditionalPatternEvaluator(
        df=df, forward_bars=10, profit_threshold=0.01  # 10 barras = 2.5 horas en 15min  # 1% profit
    )
    ...
    pattern2_variants.append(
        Pattern(
            ...
            conditions=[Condition(ConditionType.VOLUME_HIGH, {"multiplier": 2.0})],
            require_all=True,
        )
    )
    ...
    pattern2_variants.append(
        Pattern(
            ...
            conditions=[Condition(ConditionType.PRICE_MOVEMENT_LARGE, {"threshold_pct": 2.0})],
            require_all=True,
        )
    )
    ...
    pattern3_variants.append(
        Pattern(
            name="IFVG_Only", conditions=[Condition(ConditionType.IFVG_PRESENT, {"direction": "any"})], require_all=True
        )
    )
    ...
    for i, result in enumerate(all_results[:10], 1):
        ...
        print(
            f"   Win Rate: {result.win_rate:.2%} | Expectancy: {result.expectancy:.4f} | PF: {result.profit_factor:.2f}"
        )
        print(f"   Ocurrencias: {result.occurrences}")
        print()
    ...
    report_file = evaluator.generate_report(all_results, filename="reports/specific_patterns_evaluation.md")
