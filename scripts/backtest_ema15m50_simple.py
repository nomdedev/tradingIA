...
def apply_ema15m50_filter(df_with_signals: pd.DataFrame, proximity_pct: float = 2.0) -> pd.DataFrame:
    ...
    df_with_signals["ema15m50_distance_pct"] = (
        abs(df_with_signals["close"] - df_with_signals["ema15m50"]) / df_with_signals["ema15m50"] * 100
    )

    # Mark bars near EMA
    df_with_signals["near_ema15m50"] = df_with_signals["ema15m50_distance_pct"] <= proximity_pct

    return df_with_signals


def run_backtest_with_strategy(
    strategy_class, strategy_name: str, df: pd.DataFrame, apply_ema_filter: bool = False, initial_capital: float = 10000
) -> dict:
    """
...
def main():
    ...
    squeeze_original = run_backtest_with_strategy(
        SqueezeMomentumADXTTMStrategy, "Squeeze+ADX+TTM (Original)", df.copy(), apply_ema_filter=False
    )
    all_results["squeeze_original"] = squeeze_original

    squeeze_enhanced = run_backtest_with_strategy(
        SqueezeMomentumADXTTMStrategy, "Squeeze+ADX+TTM+EMA15m50", df.copy(), apply_ema_filter=True
    )
