...
try:
    ...
    result = backtester._calculate_realistic_execution_price(
        base_price=50000, order_size=1.0, avg_volume=10.0, volatility=0.02, side="buy", timestamp=None
    )
...
try:
    ...
    for order_size, avg_volume, desc in test_cases:
        impact = model.calculate_impact(
            order_size=order_size, avg_volume=avg_volume, price=50000, volatility=0.02, bid_ask_spread=10  # $10 spread
        )
...
try:
    ...
    order1 = Order(order_id="TEST1", symbol="BTCUSD", side=OrderSide.BUY, order_type=OrderType.MARKET, quantity=1.0)
