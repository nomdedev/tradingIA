...
class AdvancedBacktester:
        ...
        for idx in range(len(df_5m)):
            try:
                ...
                vp_data = vp_cache.get(
                    idx,
                    {"poc": df_5m.iloc[idx]["close"], "vah": df_5m.iloc[idx]["high"], "val": df_5m.iloc[idx]["low"]},
                )
                ...
                if idx % 1000 == 0:
                    entries_so_far = sum(
                        1 for i in range(idx + 1) if i < len(signals["entries"]) and signals["entries"].iloc[i] == 1
                    )
                ...
                if idx < len(signals["exits"]) and signals["exits"].iloc[idx] == 1:
                    signal = {
                        "action": "exit",
                        "direction": (
                            -signals["signals"].iloc[idx]
                            if idx < len(signals["signals"]) and signals["signals"].iloc[idx] != 0
                            else 0
                        ),
                    }
    ...
    def _calculate_metrics(self):
        ...
        sharpe_ratio = (
            ...
            if len(returns) > 0 and excess_returns.std() > 0
            else 0.0
        )
...
def run_advanced_backtest():
    ...
    with open("results/advanced_squeeze_adx_poc_analysis.md", "w", encoding="utf-8") as f:
        ...
        f.write("|---|---|---|---|---|---|\n")
        f.write(
            f"| **Full** | {results['full']['total_return_pct']:.2f}% | {results['full']['total_trades']} | {results['full']['win_rate']:.2f}% | {results['full']['poc_trades']} | {results['full']['poc_win_rate']:.2f}% |\n"
        )
        f.write(
            f"| **Basic** | {results['basic']['total_return_pct']:.2f}% | {results['basic']['total_trades']} | {results['basic']['win_rate']:.2f}% | - | - |\n"
        )
        f.write(
            f"| **Diferencia** | {mejora:.2f}% | {results['full']['total_trades'] - results['basic']['total_trades']} | {(results['full']['win_rate'] - results['basic']['win_rate']):.2f}% | - | - |\n\n"
        )
