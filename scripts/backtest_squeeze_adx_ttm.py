...
def load_market_data(
    symbol: str = "BTC/USD", timeframe: str = "5Min", start_date: str = "2024-01-01", end_date: str = "2025-11-12"
) -> pd.DataFrame:
    """
...
def run_comparative_backtest(strategy: SqueezeMomentumADXTTMStrategy, df_multi_tf: Dict[str, pd.DataFrame]) -> Dict:
    """
    ...
    print(".2f")
    print(
        "   Signal reduction: {} ({:.1%})".format(
            improvement["signal_reduction"],
            (
                improvement["signal_reduction"] / len(signals_no_mtf[signals_no_mtf != 0])
                if len(signals_no_mtf[signals_no_mtf != 0]) > 0
                else 0
            ),
        )
    )
    ...
    results = {
        "without_multi_tf": {"metrics": metrics_no_mtf, "signal_count": len(signals_no_mtf[signals_no_mtf != 0])},
        "with_multi_tf": {"metrics": metrics_with_mtf, "signal_count": len(signals_with_mtf[signals_with_mtf != 0])},
