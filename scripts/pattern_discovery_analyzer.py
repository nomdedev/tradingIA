...
class PatternDiscoveryAnalyzer:
    ...
    def analyze_ema_proximity_patterns(
        self,
        df: pd.DataFrame,
        ...
        for period in ema_periods:
            for threshold in proximity_thresholds:
                ...
                if bounce_up_total >= self.min_occurrences:
                    ...
                    avg_loss = (
                        df.loc[
                            (condition & (df[f"dist_ema_{period}"] < 0) & (df["future_return"] <= 0)), "future_return"
                        ].mean()
                        * 100
                    )
                    ...
                    results.append(
                        PatternResult(
                            ...
                            profit_factor=(
                                abs(avg_profit / avg_loss) if (not pd.isna(avg_loss) and avg_loss != 0) else 0
                            ),
    ...
    def analyze_volume_poc_patterns(self, df: pd.DataFrame, forward_periods: int = 20) -> List[PatternResult]:
        """
        ...
        for condition, name, expected_dir in [
            (strong_buy, "Desbalance COMPRA >2x", 1),
            ...
            if total >= self.min_occurrences:
                avg_profit = abs(
                    df.loc[condition & (np.sign(df["future_return"]) == expected_dir), "future_return"].mean() * 100
                )
                avg_loss = abs(
                    df.loc[condition & (np.sign(df["future_return"]) != expected_dir), "future_return"].mean() * 100
                )
    ...
    def analyze_ifvg_patterns(self, df: pd.DataFrame, forward_periods: int = 20) -> List[PatternResult]:
        """
        ...
        # Tamaño del gap
        df["ifvg_bull_size"] = np.where(df["ifvg_bullish"], (df["low"] - df["high"].shift(2)) / df["close"] * 100, 0)
        df["ifvg_bear_size"] = np.where(df["ifvg_bearish"], (df["low"].shift(2) - df["high"]) / df["close"] * 100, 0)
    ...
    def analyze_squeeze_momentum_patterns(self, df: pd.DataFrame, forward_periods: int = 20) -> List[PatternResult]:
        """
    ...
    def analyze_multitimeframe_patterns(self, df_5min: pd.DataFrame, forward_periods: int = 20) -> List[PatternResult]:
        """
        ...
        for tf_df, name in [(df_15min, "15min"), (df_1h, "1h")]:
            ...
            tf_df[f"trend_{name}"] = np.where(
                tf_df[f"ema_20_{name}"] > tf_df[f"ema_50_{name}"],
                1,
    ...
    def generate_pattern_report(
        self, all_results: Dict[str, List[PatternResult]], min_profit_factor: float = 1.5, min_win_rate: float = 0.55
    ) -> str:
        """
        ...
        long_patterns = [
            p
            for _, p in best_patterns
            if "LARGO" in p.condition or "ALCISTA" in p.condition or "REBOTE ALCISTA" in p.condition
        ]
        if long_patterns:
        ...
        short_patterns = [
            p
            for _, p in best_patterns
            if "CORTO" in p.condition or "BAJISTA" in p.condition or "RECHAZO BAJISTA" in p.condition
        ]
        if short_patterns:
...
def run_pattern_discovery(df: pd.DataFrame, min_occurrences: int = 10, output_file: str = None):
    """
