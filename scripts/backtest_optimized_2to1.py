...
class OptimizedBacktester:
    ...
    def run_optimized_backtest(self, df_5m, df_15m=None, df_1h=None, target_rr_ratio=2.0):
        ...
        df_full["atr"] = talib.ATR(
            df_full["high"].values, df_full["low"].values, df_full["close"].values, timeperiod=14
        )
    ...
    def _should_enter_trade(self, df, idx, current_bar):
        ...
        if idx % 500 == 0:
            print(
                f"Debug idx {idx}: entries len={len(signals['entries'])}, last_entry={signals['entries'].iloc[-1] if len(signals['entries']) > 0 else 'N/A'}"
            )
    ...
    def _enter_trade(self, bar):
        ...
        if position_value * (1 + self.commission) > self.capital:
            print(
                f"❌ Insufficient capital: need ${position_value * (1 + self.commission):.2f}, have ${self.capital:.2f}"
            )
...
def optimize_strategy_parameters():
    ...
    for i, (adx_min, squeeze_min, stop_mult, take_mult, cooldown, trailing_activation, ema_filter) in enumerate(
        param_combinations
    ):
        print(
            f"🧪 Test {i+1}/7: ADX({adx_min}), Squeeze>{squeeze_min}, SL:{stop_mult}ATR, TP:{take_mult}ATR, Cooldown:{cooldown}, EMA:{ema_filter}"
        )
...
def generate_parameter_impact_analysis(results):
    ...
    for param, data in sorted_params[:5]:
        impact = (
            "🔺 Positivo"
            if data["me_correlation"] > 0.3
            else "🔻 Negativo" if data["me_correlation"] < -0.3 else "➡️ Neutral"
        )
    ...
    with open("results/parameter_impact_analysis.md", "w", encoding="utf-8") as f:
        ...
        for param, data in param_impacts.items():
            f.write(
                f"| {param} | {data['range']} | {data['me_correlation']:.3f} | {data['win_rate_correlation']:.3f} | {data['profit_factor_correlation']:.3f} | {data['best_value']} |\n"
            )
...
def print_comparison_table(results):
    ...
    for metric_name, metric_key, format_str, suffix in metrics_to_compare:
        ...
        print(
            f"{metric_name:<25} {val1:{format_str}}{suffix:<10} {val2:{format_str}}{suffix:<10} {diff_color} {diff:{format_str}}{suffix}"
        )
