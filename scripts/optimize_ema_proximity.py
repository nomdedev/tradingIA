...
def apply_ema_proximity_filter_to_trades(df: pd.DataFrame, trades: list, proximity_pct: float) -> tuple:
    """
...
def run_backtest_with_proximity(df: pd.DataFrame, proximity_pct: float, initial_capital: float = 10000) -> dict:
    ...
    backtester = OptimizedBacktester(strategy=strategy, initial_capital=int(initial_capital))
    ...
    filtered_trades, filter_stats = apply_ema_proximity_filter_to_trades(df.copy(), all_trades, proximity_pct)

    # Calculate metrics for filtered trades
    metrics = calculate_metrics_from_trades(filtered_trades, initial_capital)

    # Combine results
    results = {"proximity_pct": proximity_pct, **metrics, **filter_stats}
...
def plot_optimization_results(results_df: pd.DataFrame, output_dir: Path):
    ...
    ax1.plot(
        best_wr["proximity_pct"], best_wr["win_rate"], "r*", markersize=20, label=f'Mejor: {best_wr["proximity_pct"]}%'
    )

    # 2. Profit Factor vs Proximity
    ax2 = axes[0, 1]
    ax2.plot(results_df["proximity_pct"], results_df["profit_factor"], "o-", linewidth=2, markersize=8, color="green")
    ...
    ax3 = axes[1, 0]
    ax3.plot(results_df["proximity_pct"], results_df["total_trades"], "o-", linewidth=2, markersize=8, color="orange")
    ...
    ax4 = axes[1, 1]
    ax4.plot(results_df["proximity_pct"], results_df["expectancy"], "o-", linewidth=2, markersize=8, color="purple")
    ...
    ax5 = axes[2, 0]
    ax5.plot(
        results_df["proximity_pct"], results_df["bars_near_ema_pct"], "o-", linewidth=2, markersize=8, color="brown"
    )
    ...
    results_df_norm["exp_norm"] = np.clip(
        (results_df["expectancy"] / results_df["expectancy"].abs().max()) * 50 + 50, 0, 100
    )

    # Composite score: average of normalized metrics
    results_df_norm["composite_score"] = (
        results_df_norm["wr_norm"] * 0.4 + results_df_norm["pf_norm"] * 0.4 + results_df_norm["exp_norm"] * 0.2
    )

    ax6.plot(
        results_df_norm["proximity_pct"],
        results_df_norm["composite_score"],
        "o-",
        linewidth=2,
        markersize=8,
    ...
    ax6.plot(
        best_comp["proximity_pct"],
        best_comp["composite_score"],
        "r*",
        markersize=20,
...
def main():
    ...
    results_df["exp_norm"] = np.clip(
        (results_df["expectancy"] / results_df["expectancy"].abs().max()) * 50 + 50, 0, 100
    )
    results_df["composite_score"] = (
        results_df["wr_norm"] * 0.4 + results_df["pf_norm"] * 0.4 + results_df["exp_norm"] * 0.2
    )
    ...
    top_10 = results_df.nlargest(10, "composite_score")[
        ["proximity_pct", "total_trades", "win_rate", "profit_factor", "expectancy", "composite_score"]
    ]
