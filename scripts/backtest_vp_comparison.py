...
def run_comparative_backtest():
    ...
    with open("results/vp_impact_analysis.md", "w", encoding="utf-8") as f:
        ...
        f.write("|---|---|---|---|---|---|\n")
        f.write(
            f"| **Con VP** | {results['con_vp']['total_return_pct']:.2f}% | {results['con_vp']['win_rate']:.2f}% | {results['con_vp']['profit_factor']:.2f} | ${results['con_vp']['expectancy']:.2f} | {results['con_vp']['total_trades']} |\n"
        )
        f.write(
            f"| **Sin VP** | {results['sin_vp']['total_return_pct']:.2f}% | {results['sin_vp']['win_rate']:.2f}% | {results['sin_vp']['profit_factor']:.2f} | ${results['sin_vp']['expectancy']:.2f} | {results['sin_vp']['total_trades']} |\n"
        )
        f.write(
            f"| **Diferencia** | {mejora:.2f}% | {(results['con_vp']['win_rate'] - results['sin_vp']['win_rate']):.2f}% | {(results['con_vp']['profit_factor'] - results['sin_vp']['profit_factor']):.2f} | ${(results['con_vp']['expectancy'] - results['sin_vp']['expectancy']):.2f} | {results['con_vp']['total_trades'] - results['sin_vp']['total_trades']} |\n\n"
        )
