...
def create_multi_tf_data(df_5min: pd.DataFrame) -> dict:
    ...
    return {"5Min": df_5min, "15Min": df_15min, "1H": df_1h}
...
def calculate_returns(df: pd.DataFrame, signals: pd.Series) -> Dict:
    ...
    return {"total_return": total_return, "win_rate": win_rate, "total_trades": total_trades, "sharpe_ratio": sharpe}
...
def run_backtest_comparison():
    ...
    for config in configs:
        ...
        results.append({"config": config["name"], "metrics": metrics, "signals": signals})
    ...
    for result in results:
        ...
        report.append(
            f"| {result['config']} | {metrics['total_return']:.4f} | {metrics['win_rate']:.1%} | {metrics['total_trades']} | {metrics['sharpe_ratio']:.2f} |"
        )
