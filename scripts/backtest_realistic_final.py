...
class RealisticBacktester:
        ...
        for idx in range(len(df_full)):
            ...
            try:
                ...
                if (
                    ...
                    and signals["entries"].iloc[idx]
                    and self._can_enter_trade(df_full.iloc[idx], df_full.iloc[idx]["poc"])
                ):
    ...
    def _exit_trade(self, bar):
        ...
        trade.update({"exit_price": exit_price, "pnl": pnl, "exit_time": bar.name, "status": "closed"})
...
def run_realistic_analysis():
    ...
    with open("results/realistic_backtest_final.md", "w", encoding="utf-8") as f:
        ...
        f.write("|---|---|---|---|\n")
        f.write(
            f"| Básica | {results['basic']['total_return_pct']:.2f}% | {results['basic']['total_trades']} | {results['basic']['win_rate']:.1f}% | - |\n"
        )
        f.write(
            f"| Completa | {results['full']['total_return_pct']:.2f}% | {results['full']['total_trades']} | {results['full']['win_rate']:.1f}% | - |\n"
        )
        f.write(
            f"| Diferencia | {mejora:.2f}% | {results['full']['total_trades'] - results['basic']['total_trades']} | {(results['full']['win_rate'] - results['basic']['win_rate']):.1f}% | - |\n\n"
        )
