...
def analyze_ema_proximity_behavior(df_5m, df_higher_tf, ema_period, timeframe_name, proximity_pct=2.0):
    ...
    analysis = {
        ...
        "squeeze_on_near_ema_pct": (
            (near_ema_data["squeeze_on"].sum() / len(near_ema_data) * 100)
            if "squeeze_on" in near_ema_data.columns and len(near_ema_data) > 0
            else 0
        ),
        "squeeze_on_overall_pct": (
            (df_5m["squeeze_on"].sum() / len(df_5m) * 100) if "squeeze_on" in df_5m.columns else 0
        ),
        # Momentum behavior near EMA
        "avg_squeeze_momentum_near_ema": (
            near_ema_data["squeeze_momentum"].mean() if "squeeze_momentum" in near_ema_data.columns else 0
        ),
...
def plot_ema_analysis(ema_config, analysis, metrics, backtester, save_path):
    ...
    # Title
    fig.suptitle(
        f"Análisis de Proximidad a {config_name} (±{ema_config['proximity']}%)\n"
    ...
    ax1.fill_between(
        ...
        equity_curve,
        10000,
        where=(np.array(equity_curve) >= 10000),
        alpha=0.3,
        color="green",
        label="Profit",
    )
    ...
    bars_prox = ax5.bar(range(len(proximity_stats)), list(proximity_stats.values()), color=colors_prox, alpha=0.7)
    ...
    if trade_returns:
        ...
        ax6.fill_between(
            ...
            cumulative_returns,
            0,
            where=(np.array(cumulative_returns) >= 0),
            alpha=0.3,
            color="green",
        )
        ax6.fill_between(
    ...
    table = ax7.table(cellText=metrics_table, cellLoc="left", loc="center", colWidths=[0.5, 0.5])
