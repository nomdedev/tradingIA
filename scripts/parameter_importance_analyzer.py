...
# Conditional import for sklearn (not compatible with Python 3.14+)
try:
...
class ParameterImportanceAnalyzer:
    ...
    def _calculate_performance_metrics(self, signals: pd.Series, prices: pd.Series) -> Dict:
        ...
        if not returns:
            return {"total_return": 0, "win_rate": 0, "total_trades": 0, "avg_trade": 0, "sharpe_ratio": 0}
    ...
    def analyze_parameter_importance(self, df_multi_tf: Dict[str, pd.DataFrame], parameter_ranges: Dict) -> Dict:
        """
        ...
        for param_name, param_values in parameter_ranges.items():
            ...
            for value in param_values:
                ...
                if not signals.empty and "5Min" in df_multi_tf:
                    ...
                    print(
                        f"  Value {value}: Return={metrics['total_return']:.4f}, "
    ...
    def generate_parameter_report(self, importance_results: Dict, multi_tf_results: Dict) -> str:
        ...
        for param_name, param_results in importance_results.items():
            ...
            if param_results:
                ...
                best_result = max(param_results, key=lambda x: x["metrics"]["total_return"])
                worst_result = min(param_results, key=lambda x: x["metrics"]["total_return"])

                report.append(
                    f"**Best Value**: {best_result['value']} "
                    f"(Return: {best_result['metrics']['total_return']:.4f}, "
                    f"Win Rate: {best_result['metrics']['win_rate']:.2%})"
                )
                report.append(
                    f"**Worst Value**: {worst_result['value']} "
        ...
        report.append(
            "2. **Multi-timeframe confirmation** appears to "
            + ("improve" if multi_tf_results.get("improvement", {}).get("total_return", 0) > 0 else "not improve")
            + " performance"
        )
    ...
    def save_results(self, importance_results: Dict, multi_tf_results: Dict, filename: str = None) -> None:
...
def run_parameter_importance_analysis(
    strategy: SqueezeMomentumADXTTMStrategy, df_multi_tf: Dict[str, pd.DataFrame]
) -> None:
    """
