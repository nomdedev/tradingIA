...
def calculate_metrics(trades_df):
    ...
    if not trades_df.empty and "entry_time" in trades_df.columns and "exit_time" in trades_df.columns:
        ...
        avg_losing_duration = (
            np.mean([d for d, w in zip(durations, winning_mask) if not w]) if not winning_mask.all() else 0
        )
    else:
...
def test_weight_combinations():
    ...
    for i, weights in enumerate(weight_configs):
        ...
        backtest_result = backtester.run_backtest(
            data["5Min"], signals["entries"], signals["exits"], strategy_name=f"Weighted_Config_{i+1}"
        )
...
def run_optimized_backtest(best_weights=None):
    ...
    backtester = SimpleBacktester(strategy, initial_capital=10000)
    backtest_result = backtester.run_backtest(
        data["5Min"], signals["entries"], signals["exits"], strategy_name="Weighted_Multi_Strategy_Optimized"
    )
