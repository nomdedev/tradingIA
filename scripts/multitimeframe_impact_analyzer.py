...
class MultiTimeframeImpactAnalyzer:
    ...
    def _calculate_trend_alignment(self, df_5min: pd.DataFrame, df_higher: pd.DataFrame) -> pd.Series:
        ...
        df_higher["trend"] = np.where(
            df_higher["ema_fast"] > df_higher["ema_slow"],
            1,
    ...
    def analyze_timeframe_impact(self, df_5min: pd.DataFrame, higher_timeframes: List[str] = ["15Min", "1H"]) -> Dict:
        """
    ...
    def generate_timeframe_report(self, weight_results: Dict, regime_results: Dict) -> str:
        ...
        for key, result in weight_results.items():
            ...
            report.append(
                f"| {weights['higher_tf_weight']}/{weights['lower_tf_weight']} | "
        ...
        best_result = max(weight_results.values(), key=lambda x: x["metrics"]["total_return"])
        ...
        if regime_results:
            best_regime = max(regime_results.keys(), key=lambda x: regime_results[x]["metrics"]["total_return"])
            best_metrics = regime_results[best_regime]["metrics"]
            report.append(
                f"**Best Regime:** {best_regime.title()} "
        ...
        report.append("")
        report.append(
            "1. **Optimal Multi-TF Weights:** Use the configuration that maximizes return while maintaining acceptable win rate"
        )
        report.append(
            "2. **Regime Adaptation:** Consider adjusting strategy parameters based on detected market regime"
        )
        report.append(
            "3. **Signal Filtering:** Higher timeframe confirmation reduces false signals but may miss opportunities"
        )
        report.append("4. **Further Analysis:** Test with different assets and market conditions")

        return "\n".join(report)

    def create_visualizations(self, weight_results: Dict, regime_results: Dict, save_path: str = None) -> None:
        ...
        weights = [
            f"{r['weights']['higher_tf_weight']}/{r['weights']['lower_tf_weight']}" for r in weight_results.values()
        ]
...
def run_multitimeframe_analysis(strategy: SqueezeMomentumADXTTMStrategy, df_5min: pd.DataFrame) -> None:
    """
